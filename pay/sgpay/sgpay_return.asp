...
'
 ...
 '
		...
		'		& " VALUES ( '\pay\sgpay\sgpay_return.asp', '"& order_num &"','"& giftcard_serial &"','https://api-2.bbq.co.kr/api/VoucherInfo/', '"& jsonGiftcard &"','"& postResponse &"','"& MA_RTN_CD '&"','"& MA_RTN_MSG &"', GETDATE() ) "
		'	dbconn.Execute(Sql)
'
'
		'	' 상품권 사용처리 data set
...
'
		...
		'		  
		'			Sql = " INSERT INTO bt_giftcard_log(source_id, order_num, giftcard_no, api_nm, in_param, out_param, MA_RTN_CD, MA_RTN_MSG, regdate) "_
		'				& " VALUES ( '\pay\sgpay\sgpay_return.asp', '"& order_num &"','"& giftcard_serial &"','https://api-2.bbq.co.kr/api/VoucherUse/', '"& jsonGiftcard &"','"& gpostResponse &"','"& 'MA_RTN_CD &"','"& MA_RTN_MSG &"', GETDATE() ) "
		'			dbconn.Execute(Sql)
		'		  
		'			If MA_RTN_CD = "0000" THEN 				 
...
'
		...
		'		'쿠키 제거
		'	Else
		'		ErrMessage = "상품권 사용에 실패하였습니다. (" & RTN_MSG & ")"
		'	End   ' If MA_RTN_CD = "0000" Then
'
		'End If   ' If jsonGiftcard <> "" Then
