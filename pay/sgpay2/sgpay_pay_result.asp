...
<%
	...
	if resultCode = "0000" then
		...
		if not (signature = signatureCheck) then
			...
			result = PayCancel(g_corpMemberNo, userMngNo, stdPayUniqNo, "결제정보 변경에 의한 취소")
								...
								If MA_RTN_CD = "0000" THEN 				 
									...
									For Each row In this.item("Voucher_Use").item("data")
										...
										With pCmd
											...
											.Parameters.Append .CreateParameter("@pay_transaction_id", adVarChar, adParamInput, 50, this1.item("SN"))
											...
											.Parameters.Append .CreateParameter("@pay_amt", adCurrency, adParamInput, , this1.item("AMT"))
