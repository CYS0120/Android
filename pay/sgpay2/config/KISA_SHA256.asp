...
Private Function LShift_SHA(lValue, iShiftBits)
    If iShiftBits = 0 Then
        LShift_SHA = lValue
        Exit Function
    ElseIf iShiftBits = 31 Then
        If lValue And 1 Then
            LShift_SHA = &H80000000
        Else
            LShift_SHA = 0
        End If
        Exit Function
    ...
    If (lValue And m_l2Power(31 - iShiftBits)) Then
        LShift_SHA = ((lValue And m_lOnBits(31 - (iShiftBits + 1))) * m_l2Power(iShiftBits)) Or &H80000000
    Else
        LShift_SHA = ((lValue And m_lOnBits(31 - iShiftBits)) * m_l2Power(iShiftBits))
    End If
End Function

Private Function RShift_SHA(lValue, iShiftBits)
    If iShiftBits = 0 Then
        RShift_SHA = lValue
        Exit Function
    ElseIf iShiftBits = 31 Then
    ...
    RShift_SHA = (lValue And &H7FFFFFFE) \ m_l2Power(iShiftBits)
    
    If (lValue And &H80000000) Then
        RShift_SHA = (RShift_SHA Or (&H40000000 \ m_l2Power(iShiftBits - 1)))
    End If
End Function
...
Private Function S(x, n)
    S = (RShift_SHA(x, (n And m_lOnBits(4))) Or LShift_SHA(x, (32 - (n And m_lOnBits(4)))))
End Function

Private Function R(x, n)
    R = RShift_SHA(x, CInt(n And m_lOnBits(4)))
End Function
...
Private Function SHA256_Process(sMessage)
    ...
    Do Until lByteCount >= lMessageLength
        ...
        lWordArray(lWordCount) = lWordArray(lWordCount) Or LShift_SHA(lByte, lBytePosition)
    ...
    lWordArray(lWordCount) = lWordArray(lWordCount) Or LShift_SHA(&H80, lBytePosition)

    lWordArray(lNumberOfWords - 1) = LShift_SHA(lMessageLength, 3)
    lWordArray(lNumberOfWords - 2) = RShift_SHA(lMessageLength, 29)
