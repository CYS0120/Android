...
<%
	...
	'******************************************************/
    RETURNURL = GetCurrentHost& "/pay/kakaopay/CPCGI.asp"& param_str
    CANCELURL = GetCurrentHost& "/pay/kakaopay/Cancel.asp"
    ...
    REQ_DATA.Add "QUOTA", "00"  'USESKIPPAGE 필수
	...
	'* 제휴사 정보
	'**************************************************/
    REQ_DATA.Add "ALLIANCECODEBASE", "KAKAO"

    '**************************************************
	'* 표준결제창 정보
	'**************************************************/
    REQ_DATA.Add "USESKIPPAGE", "Y"
...
' ISDEBUG = TRUE
    ...
    Sql = "Insert Into bt_order_g2_log(order_idx, payco_log, coupon_amt, log_point) values('"& order_idx &"','"& Session("UserId") &"','0','kakaopay-session-ready')"
    dbconn.Execute(Sql)
