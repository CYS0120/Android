...
'		Response.write oRs("ORDER_IDX") & " | " & oRs("ORDER_ID") & " | " & oRs("PAY_TYPE") & " | " & oRs("PAY_AMT") & " | " & oRs("TID") & " | " & oRs("BRANCH_ID") & " | " & oRs("CUST_ID") & "<BR>"
	...
	' 2021-07 더페이 상품권 복수 처리 기존 소스 주석 시작 
    ''상품권 취소
    ...
    '    End If
    'End If
	' 2021-07 더페이 상품권 복수 처리 기존 소스 주석 끝 
	 

	' // --------------------------------------------------------------------------------------------------------
	' 2021-07 더페이 상품권 복수 처리 수정 시작 
    '상품권 취소
    '사용 상품권 조회 (dbo.bt_giftcard)
    CountQuery = " SELECT COUNT(*) OVER() as cnt, u_cd_brand as U_CD_BRAND, u_cd_partner as U_CD_PARTNER, giftcard_amt as AMT, ok_num as OK_NUM, giftcard_number as Gift_Serial " _ 
	           & "   FROM BBQ.dbo.bt_giftcard WITH(NOLOCK) " _ 
    ...
    If Not (Gift_Count.BOF Or Gift_Count.EOF) Then
		...
		If Gift_Count("cnt") > 0 Then 
			...
			dim cancelSN : cancelSN = ""  
			dim giftcard_serial : giftcard_serial = ""

			Do Until Gift_Count.EOF 
				If jsonGiftcard = "" Then
					jsonGiftcard = "{ ""U_CD_BRAND"":""" &  Gift_Count("U_CD_BRAND") & """," _
								   & """U_CD_PARTNER"":""" &  Gift_Count("U_CD_PARTNER") & """," _
								   & """ORDER_ID"":""" & ORDER_ID & """," _
								   & """SRV"":""HOMEPAGE""," _
								   & """GIFTCARD"":["				                   
					jsonGiftcard = jsonGiftcard & "{""SN"":""" & Gift_Count("Gift_Serial")  & """, ""AMT"":""" & Gift_Count("AMT") & """, ""OK_NUM"":""" & Gift_Count("OK_NUM") & """}"
				Else
					jsonGiftcard = jsonGiftcard & ",{""SN"":""" & Gift_Count("Gift_Serial")  & """, ""AMT"":""" & Gift_Count("AMT") & """, ""OK_NUM"":""" & Gift_Count("OK_NUM") & """}"
				End If   

				cancelSN = cancelSN & "'" & Gift_Count("Gift_Serial") & "',"
				giftcard_serial = giftcard_serial & Gift_Count("Gift_Serial") & "||"
				Gift_Count.MoveNext
			LOOP
			...
			If cancelSN <> "" Then
				cancelSN = cancelSN & ","
				cancelSN = Replace(cancelSN, ",,", "")
				giftcard_serial = giftcard_serial & "||"
				giftcard_serial = Replace(giftcard_serial, "||||", "")
			End If

'	Response.Write "^ cancelSN ::: " & cancelSN & "<BR>"
			...
			gJSON.loadJSON(gpostResponse)
			Set this = gJSON.data("list")  
			RTN_CD  = this.item("Voucher_Cancel").item("MA_RTN_CD") '  
			RTN_MSG = this.item("Voucher_Cancel").item("MA_RTN_MSG") ' 사용여부
			'사용 상품권 text -> json 
		...
		'Response.Write "^" & RTN_CD & " --- " & cancelSN & " ::: " & gpostResponse
			...
			If RTN_CD = "0000" Then
				...
				Sql = "UPDATE BBQ.dbo.bt_giftcard SET u_cd_brand = null, u_cd_partner = null, ok_num = null, order_num = null, used_date = null " _
	                    & " WHERE giftcard_number IN (" & cancelSN & ") "  
				dbconn.Execute(Sql)
	
		'Response.Write "^" & RTN_CD & " --- " & Sql & " ::: " & gpostResponse
			...
			Else
				...
				Response.Write html_result
				Response.End
			End If   ' If RTN_CD = "0000" Then
			'상품권 사용처리 
 

		End If   ' If Gift_Count("cnt") > 0 Then
   
    End If
	' 2021-07 더페이 상품권 복수 처리 수정 끝 
	' // --------------------------------------------------------------------------------------------------------
