...
function CheckInput(tp, num){
	if (tp == "update") {
		...
		var cnt = $("#cnt").val();
		for (var i = 1; i <= cnt; ++i) {
			var fee = "#add_delFee" + i;
			var dong = "#dong" + i;
			// alert(i+" @@ "+$(dong).val());
			if($(dong).val() == "undefined" || $(dong).val() == "" || $(dong).val() == null){alert("동을 입력해주세요");$(dong).focus();return;}
			if($(fee).val() <= 0 || $(fee).val() == "" || $(fee).val() == null){alert("추가배달료를 입력해주세요");$(fee).focus();return;}
		}
		...
		for (var i = 1; i <= cnt; ++i) {
			...
			for (var j = i+1; j <= cnt; ++j) {
				...
				if ($(dong1).val() == $(dong2).val()){alert("하나의 동에 추가배달료 한 번만 설정해주세요");$(dong2).focus();return;}
			}
		}
	} else if (tp == "delete") {
	...
	$.ajax({
		...
		data: $("#inputfrm").serialize()+"&tp="+tp+"&num="+num,
		dataType: "text",
		success: function (data) {
			alert(data);
			$('.mask, .window').hide();
