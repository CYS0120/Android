...
<%										End If 
										If MENU_FILE = "stats_order.asp" Then %>
											<li><label><input type="radio" name="MCD" checked onClick="document.location.href='stats_order.asp?CD=<%=CD%>'"><span>주문통계</span></label>
											( 목록형태:
...
<%						End If %>
					...
					<div class="db db_1">
<%
	SqlWhere = " WHERE TB_I.cdate >= '"& Replace(SDATE,"-","") &"' And TB_I.cdate <= '"& Replace(EDATE,"-","") &"' AND TB_I.[ORD_TYPE] = '10'"
	'''목록 확인을 위해서 주석
...
<%
		...
		If SCD = "M" Then 
			...
			SqlFrom = SqlFrom & "		( 	"
			SqlFrom = SqlFrom & "			SELECT BRAND_CODE, TB_I.[order_id], product_id, items, list_price, disc_price, TB_I.ctime, TB_I.brand_id, BT_O.pay_type	"
			SqlFrom = SqlFrom & "			FROM [TB_WEB_ORDER_ITEM] TB_I WITH(NOLOCK)	"
			SqlFrom = SqlFrom & "			LEFT JOIN [TB_WEB_ORDER_STATE] TB_S WITH(NOLOCK) ON TB_I.[order_id] = TB_S.[order_id] 	"
			SqlFrom = SqlFrom & "			LEFT JOIN [BT_ORDER] BT_O WITH(NOLOCK) ON TB_I.[order_id] = BT_O.[order_num] 	"
			SqlFrom = SqlFrom & SqlWhere	
			SqlFrom = SqlFrom & "		) AS A	"
			SqlFrom = SqlFrom & "	) AS E 	"
			SqlFrom = SqlFrom & "	GROUP BY BRAND_CODE, pay_type	"
		ElseIf SCD = "A" Then 
			SqlFrom = SqlFrom & "		SELECT TB.BRAND_CODE, TB.BRANCH_ID, BRANCH_NAME as TITLE_NAME, items, list_price, disc_price, ptime, brand_id, pay_type FROM	"
			SqlFrom = SqlFrom & "		( 	"
		...
		Else
			SqlFrom = SqlFrom & "		SELECT TB.BRAND_CODE, menu_name as TITLE_NAME, base_id, items, list_price, disc_price, ptime, state, brand_id, cdate, pay_type FROM	"
			SqlFrom = SqlFrom & "		( 	"
			SqlFrom = SqlFrom & "			SELECT LEFT(ctime,2) AS ptime, * FROM  	"
			SqlFrom = SqlFrom & "			( 	"
...
'RESPONSE.END
		...
		If Not Rlist.Eof Then 
			Do While Not Rlist.Eof
				...
				GOOGLE_CHART = GOOGLE_CHART & ",['"& TITLE_NAME & "',"& SUM_COUNT &"]"
%>
									 <tr>
