...
{
	document.getElementById("ifrExcel").src = "coupon_partner_pin_excel.asp?CPNID=<%=CPNID%>";
}
</script>
</head>
<body>
...
<%
	...
	Sql = "Select COUNT(*) CNT FROM "& BBQHOME_DB &".DBO.T_CPN_MST T1 WITH(NOLOCK) INNER JOIN "& BBQHOME_DB &".DBO.T_CPN T2 WITH(NOLOCK) ON T2.CPNID = T1.CPNID INNER JOIN "& BBQHOME_DB &".DBO.T_CPN_PARTNER T3 WITH(NOLOCK) ON T3.CD_PARTNER = T2.CD_PARTNER AND T3.ADM_ID = '"& Admin_RID &"'" & SqlWhere
...
<%
	If total_num > 0 Then 
		...
		Sql = Sql & "		T1.CPNID, T2.CPNNAME, T1.PINID, T1.PIN, T1.CPNTYPE, T1.USESDATE, T1.USEEDATE, T1.STATUS, T1.PUBDATE, T1.USEDATE, T1.OWNERID, T1.TELNO, T1.U_CD_PARTNER AS BRANCH_ID "
		Sql = Sql & "	FROM "& BBQHOME_DB &".DBO.T_CPN_MST T1 WITH(NOLOCK) INNER JOIN "& BBQHOME_DB &".DBO.T_CPN T2 WITH(NOLOCK) ON T2.CPNID = T1.CPNID INNER JOIN "& BBQHOME_DB &".DBO.T_CPN_PARTNER T3 WITH(NOLOCK) ON T3.CD_PARTNER = T2.CD_PARTNER AND T3.ADM_ID = '"& Admin_RID &"'"
		Sql = Sql & SqlWhere
		Sql = Sql & ") T1 "
