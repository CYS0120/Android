...
<%
	...
	SqlFrom = "	From "& BBQHOME_DB &".DBO.T_CPN_MST MST  WITH(NOLOCK) INNER JOIN "& BBQHOME_DB &".DBO.T_CPN CPN WITH(NOLOCK) ON MST.CPNID = CPN.CPNID LEFT JOIN BT_BRANCH BRANCH WITH (NOLOCK) ON BRANCH.branch_id = MST.U_CD_PARTNER "
...
<%
	...
	'If total_num > 0 Then 
		...
		Sql = "SELECT  MST.*, CPN.CPNNAME, BRANCH.branch_name " & SqlFrom & SqlWhere & vbCrLf
