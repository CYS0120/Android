...
<%
	...
	' 테스트매장 제외
	SqlWhere	= SqlWhere & " AND A.BRANCH_ID NOT IN ('9901001') "
...
<%
	...
	If total_num > 0 Then 
		...
		Sql = Sql & "			, A.RESERV_DATE + '' + A.RESERV_TIME AS RESERVDATETIME, CASE WHEN A.ORDER_FLAG IN (2, 3, 5, 6) AND NOT (A.ORDER_FLAG = '3' AND A.ORDER_TYPE IN ('4', '5')) AND  O.order_type = 'R' THEN 'Y' ELSE 'N' END AS IS_RESERV "
...
%>
                                <tr style="background-color:<%=sRowColor%>; color:<%=sFontColor%>" onmouseover="this.style.backgroundColor='#FCFAF9';this.style.color='#000000'" onmouseout="this.style.backgroundColor='<%=sRowColor%>';this.style.color='<%=sFontColor%>'">
                                    <td><%=num%></td>
                                    <td><a href="order_web_read.asp?OID=<%=ORDER_ID%><%=Detail%>"><%=ORDER_TITLE%><%=ORDER_ID%></a></td>
                                    <td><%=ORDERTYPE_TXT%></td>
                                    <td><%=Order_flag%></td>
                                    <td><%=sOrderDate%></td>
                                    <td><%=sOrder_Time%></td>
