	...
	Else
		...
		Else 
			If IMGNUM = "W1" Or IMGNUM = "W2" Or IMGNUM = "W3" Or IMGNUM = "W4" Or IMGNUM = "W5" Then
				...
				If FncIsBlank(link_target) Then link_target = "S"
				If Not FncIsBlank(main_img) Then 
					Sql = "Update bt_main_img Set main_img='"& main_img &"', main_text='"& main_text &"', link_url='"& link_url &"', link_target='"& link_target &"', BAN_ORD='"& BAN_ORD &"', date_s='"& date_s &"', date_e='"& date_e &"', date_yn='"& date_yn &"' Where brand_code='"& brand_code &"' And main_kind='"& main_kind &"'"
					conn.Execute Sql, Uplow
					If Uplow > 0 Then 
					Else 
						Sql = "Insert Into bt_main_img(brand_code, main_kind, main_img, main_text, link_url, link_target, date_s, date_e, date_yn) values('"& brand_code &"','"& main_kind &"','"& main_img &"','"& main_text &"','"& link_url &"','"& link_target &"','"& date_s &"','"& date_e &"','"& date_yn &"')"
						conn.Execute(Sql)
					End If
				End If
