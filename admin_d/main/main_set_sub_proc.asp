...
<%
	...
	If IMGNUM = "WN" Then	'WEB 삭제 
		Sql = "Delete From bt_main_img_sub Where brand_code='"& brand_code &"' And main_kind like 'W%'"
		conn.Execute Sql
	ElseIf IMGNUM = "MN" Then	'모바일 삭제
		Sql = "Delete From bt_main_img_sub Where brand_code='"& brand_code &"' And main_kind='M1'"
		conn.Execute Sql
	Else
		If IMGNUM = "M1" Then
			main_kind	= "M1"
			...
			If Not FncIsBlank(main_img) Then 
				Sql = "Update bt_main_img_sub Set main_img='"& main_img &"', main_text='"& main_text &"', link_url='"& link_url &"', link_target='"& link_target &"', BAN_ORD='"& BAN_ORD &"' Where brand_code='"& brand_code &"' And main_kind='"& main_kind &"'"
				conn.Execute Sql, Uplow
				If Uplow > 0 Then 
				Else 
					Sql = "Insert Into bt_main_img_sub(brand_code, main_kind, main_img, main_text, link_url, link_target) values('"& brand_code &"','"& main_kind &"','"& main_img &"','"& main_text &"','"& link_url &"','"& link_target &"')"
					conn.Execute(Sql)
				End If
			End If
		Else 
			If IMGNUM = "W1" Or IMGNUM = "W2" Or IMGNUM = "W3" Or IMGNUM = "W4" Or IMGNUM = "W5" Then
				...
				If IMGNUM = "W1" Then 
					Sql = "Delete From bt_main_img_sub Where brand_code='"& brand_code &"' And main_kind IN ('W2','W3','W4','W5')"
					conn.Execute Sql
				End If
			End If
			...
			If IMGNUM = "W2" Or IMGNUM = "W3" Or IMGNUM = "W4" Or IMGNUM = "W5" Then
				...
				If IMGNUM = "W2" Then 
					Sql = "Delete From bt_main_img_sub Where brand_code='"& brand_code &"' And main_kind IN ('W3','W4','W5')"
					conn.Execute Sql
				End If
			End If
			...
			If IMGNUM = "W3" Or IMGNUM = "W4" Or IMGNUM = "W5" Then
				...
				If IMGNUM = "W3" Then 
					Sql = "Delete From bt_main_img_sub Where brand_code='"& brand_code &"' And main_kind IN ('W4','W5')"
					conn.Execute Sql
				End If
			End If
			...
			If IMGNUM = "W4" Or IMGNUM = "W5" Then
				...
				If IMGNUM = "W4" Then 
					Sql = "Delete From bt_main_img_sub Where brand_code='"& brand_code &"' And main_kind IN ('W5')"
					conn.Execute Sql
				End If
			End If
