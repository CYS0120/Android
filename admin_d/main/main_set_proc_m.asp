	...
	Else
		...
		Else 
			...
			Do while i < num
				...
				sql = sql & "EXEC UP_BT_MAIN_IMG 'MOBILE', '"& brand_code &"', '"& main_kind &"', '"& main_text &"', '"& main_img &"', '"& link_url &"', '"& link_target &"', '"& BAN_ORD &"', '"& date_s &"', '"& date_e &"', '"& date_yn &"', '"& SITE_ADM_ID &"'" & vbCrLf
			Loop

			sql = sql & "EXEC UP_BT_MAIN_IMG 'DELETE_M', '', '"& IMGNUM &"', '', '', '', '', '', '', '', '', ''"
			' response.write sql
			conn.Execute sql
