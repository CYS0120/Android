...
function InputCheck(main_kind){
	...
	$.ajax({
		...
		data: $("#inputfrm").serialize()+"&main_kind="+main_kind+"&save_all=N",
		dataType: "text",
		success: function (data) {
			alert(data.split("^")[1]);
			if(data.split("^")[0] == 'Y'){
...
function SaveAll(main_kind){
	...
	$.ajax({
		...
		type: "POST",
		url: "main_set_proc_m.asp",
		data: $("#inputfrm").serialize()+"&main_kind="+main_kind+"&save_all=Y",
		dataType: "text",
		success: function (data) {
			alert(data.split("^")[1]);
			if(data.split("^")[0] == 'Y'){
						...
						<tr>
							...
							<td width=13>
								<div style="text-align:right;margin-right:15px;">
									<input type="button" value="삭제" class="btn_red125" onClick="AdjustDiv('DEL_M', '<%=main_kind%>')">
								</div>
							</td>
						</tr>
					</table>
