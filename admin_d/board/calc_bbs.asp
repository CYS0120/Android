...
<%
	If total_num > 0 Then 
		Sql = "SELECT Top "&num_per_page&" BIDX, RNAME, RHP, STEP3, STEP4, BSTATUS, CONVERT(VARCHAR(19), REG_DATE, 121) AS REG_DATE " & SqlFrom & SqlWhere
...
%>
								<tr>
									...
									<td><span><%=RHP%></span></td>
									<td><span><%=STEP3%></span></td>
									<td><span><%=STEP4%></span></td>
									<td><span><%=REG_DATE%></span></td>
									<td><span><%=BSTATUS_TXT%></span></td>
									<td><input type="button" value="수정" class="btn_white" onClick="document.location.href='calc_bbs_form.asp?BIDX=<%=BIDX%>&CD=<%=CD%>&BBSCODE=<%=BBSCODE%>'"></td>
								</tr>
<%
				num = num - 1
				Rlist.MoveNext
