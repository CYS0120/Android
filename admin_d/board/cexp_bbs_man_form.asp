...
<%
	...
	If FncIsBlank(BIDX) Or FncIsBlank(CIDX) Then 
		Call subGoToMsg("잘못된 접근방식 입니다","back")
	End If 
	Sql = "Select C.*, M.EXPPLACE From bt_board_cexp C LEFT JOIN bt_board_cexp_main M on C.CIDX = M.CIDX Where BIDX=" & BIDX &" And C.CIDX=" & CIDX
...
<script language = "JavaScript">
	function CheckInput(MODE){
		...
		$.ajax({
			async: true,
			type: "POST",
			url: "cexp_bbs_man_form_proc.asp",
			data: $("#inputfrm").serialize(),
			dataType: "text",
			success: function (data) {
				alert(data.split("^")[1]);
				if(data.split("^")[0] == 'Y'){
					document.location.href='cexp_bbs_man.asp?CIDX=<%=CIDX%>&CD=<%=CD%>&BBSCODE=<%=BBSCODE%>';
				}
			},
...
<%	Else %>
							...
							<input type="button" class="btn_white125" onClick="CheckInput('DEL')" value="메모 삭제">
<%	End If %>
						</div>
					</div>
				</div>   
