...
<%
	...
	SqlFrom		= " From bt_global_inquiry "
	SqlWhere	= " WHERE 1=1"
	If Not FncIsBlank(SW) Then 
		If SM = "N" Then
			SqlWhere = SqlWhere & " And (FNAME like '%" & SW & "%' Or LNAME like '%" & SW & "%') "
		ElseIf SM = "E" Then
			SqlWhere = SqlWhere & " And EMAIL like '%" & SW & "%'"
		End If
	End If
...
<%
	If total_num > 0 Then 
		Sql = "SELECT Top "&num_per_page&" IDX, CANDIDATE, COUNTRY, FNAME, LNAME, EMAIL, STATUS, REG_DATE " & SqlFrom & SqlWhere
		Sql = Sql & " And IDX Not In "
		Sql = Sql & "(SELECT TOP " & ((page - 1) * num_per_page) & " IDX "& SqlFrom & SqlWhere
		Sql = Sql & SqlOrder & ")"
		Sql = Sql & SqlOrder
		...
		If Not Rlist.Eof Then 
			...
			Do While Not Rlist.Eof
				IDX	= Rlist("IDX")
				CANDIDATE	= Rlist("CANDIDATE")
				COUNTRY	= Rlist("COUNTRY")
				FNAME	= Rlist("FNAME")
				LNAME	= Rlist("LNAME")
				EMAIL	= Rlist("EMAIL")
				STATUS	= Rlist("STATUS")
...
%>
								<tr class="thum_padding">
									...
									<td><span><%=FNAME%>&nbsp;<%=LNAME%></span></td>
									<td><span><%=EMAIL%></span></td>
									<td><span><%=REG_DATE%></span></td>
									<td><span><%If STATUS="Y" Then%>답변환료<%Else%>답변대기<%End If%></span></td>
									<td><input type="button" value="수정" class="btn_white" onClick="document.location.href='global_inquiry_form.asp?IDX=<%=IDX%>&CD=<%=CD%>&BBSCODE=<%=BBSCODE%>'"></td>
								</tr>
<%
				num = num - 1
				Rlist.MoveNext
