...
<%
	...
	If FncIsBlank(BIDX) Then 
		Call subGoToMsg("잘못된 접근방식 입니다","back")
	End If 
	Sql = "Select * From bt_board_joinq Where BIDX=" & BIDX
	Set Binfo = conn.Execute(Sql)
	If Binfo.Eof Then
		Call subGoToMsg("존재하지 않는 게시물 입니다","back")
	End If 

	BOWNER_NAME	= Binfo("BOWNER_NAME")
	BEMAIL	= Binfo("BEMAIL")
	BTEL	= Binfo("BTEL")
	BHP	= Binfo("BHP")
...
<script language = "JavaScript">
	function CheckInput(MODE){
		...
		$.ajax({
			async: true,
			type: "POST",
			url: "joinq_bbs_form_proc.asp",
			data: $("#inputfrm").serialize(),
			dataType: "text",
			success: function (data) {
				alert(data.split("^")[1]);
				if(data.split("^")[0] == 'Y'){
					document.location.href='joinq_bbs.asp?CD=<%=CD%>&BBSCODE=<%=BBSCODE%>';
				}
			},
						...
						<tr>
							...
							<td>
								<select name="ASTATUS" id="ASTATUS">
									<option value="0"<% If ASTATUS = "0" Then%> selected<%End If%>>접수</option>
									<option value="1"<% If ASTATUS = "1" Then%> selected<%End If%>>상담대기</option>
									<option value="2"<% If ASTATUS = "2" Then%> selected<%End If%>>참가확인</option>
									<option value="3"<% If ASTATUS = "3" Then%> selected<%End If%>>상담진행중</option>
									<option value="4"<% If ASTATUS = "4" Then%> selected<%End If%>>상담종료</option>
								</select>
							</td>
						</tr>
						<tr>
