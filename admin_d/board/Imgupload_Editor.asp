...
<script>
	function SetImgName(IMGNAME){
		opener.$('#<%=IMGID%>').val('<%=FILE_SERVERURL%>/uploads/<%=UPDIR%>/'+IMGNAME);
		self.close();
	}
...
<body>
	<form id="inputfrm" name="inputfrm" method="POST" enctype="multipart/form-data" action="<%=FILE_SERVERURL%>/editor_save.asp" target="procframe">
