...
<%
	...
	If FncIsBlank(MIDX) Then
		Sql = "	Insert Into bt_menu_parent(brand_code, menu_type, menu_name, menu_name_e, menu_price, menu_desc, kind_sel, poscode, menu_title, " & _
			...
			"		'"& gubun_sel &"','"& option_yn &"','"& sale_shop &"','"& online_yn &"','"& smart_age &"','"& smart_taste &"', " & _
			"		'"& use_yn &"','N',( Select Isnull(Max(sort),0)+1 From bt_menu_parent Where brand_code='"& brand_code &"'),'"& origin &"','"& calorie &"','"& sugars &"','"& protein &"','"& saturatedfat &"','"& natrium &"','"& allergy &"', " & _
			"		'"& exp1_yn &"','"& exp1_url &"','"& exp2_yn &"','"& exp2_title &"','"& exp2_imgurl &"','"& exp3_yn &"','"& exp3_title &"','"& exp3_imgurl &"', " & _
			"		'"& exp4_yn &"','"& exp4_title &"','"& exp4_imgurl &"','"& exp5_yn &"','"& exp5_title &"','"& exp5_imgurl &"', " & _
			"		'"& SITE_ADM_CD &"',GetDate(),'"& REG_IP &"')	"
		conn.Execute(Sql)
		Sql = "	Select Max(menu_idx) As menu_idx From bt_menu_parent"
		Set MaxID = conn.Execute(Sql)
		menu_idx = MaxID("menu_idx")

		If Not FncIsBlank(THUMB_IMG) Then
			Sql = "	Insert Into bt_menu_parent_file(menu_idx, file_type, file_name, file_ext, file_path, del_yn, reg_user_idx, reg_date, reg_ip) " & _
		...
		Response.End
	Else
		Sql = "	Update bt_menu_parent Set brand_code='"& brand_code &"', menu_type='"& menu_type &"', menu_name='"& menu_name &"', menu_name_e='"& menu_name_e &"', menu_price='"& menu_price &"', menu_desc='"& menu_desc &"', kind_sel='"& kind_sel &"', poscode='"& poscode &"', menu_title='"& menu_title &"', gubun_sel='"& gubun_sel &"', option_yn='"& option_yn &"', sale_shop='"& sale_shop &"', online_yn='"& online_yn &"', smart_age='"& smart_age &"', smart_taste='"& smart_taste &"', use_yn='"& use_yn &"', origin='"& origin &"', calorie='"& calorie &"', sugars='"& sugars &"', protein='"& protein &"', saturatedfat='"& saturatedfat &"', natrium='"& natrium &"', allergy='"& allergy &"', exp1_yn='"& exp1_yn &"', exp1_url='"& exp1_url &"', exp2_yn='"& exp2_yn &"', exp2_title='"& exp2_title &"', exp2_imgurl='"& exp2_imgurl &"', exp3_yn='"& exp3_yn &"', exp3_title='"& exp3_title &"', exp3_imgurl='"& exp3_imgurl &"', exp4_yn='"& exp4_yn &"', exp4_title='"& exp4_title &"', exp4_imgurl='"& exp4_imgurl &"', exp5_yn='"& exp5_yn &"', exp5_title='"& exp5_title &"', exp5_imgurl='"& exp5_imgurl &"', mod_user_idx='"& SITE_ADM_CD &"', mod_date=GetDate(), mod_ip='"& REG_IP &"' Where menu_idx = " & MIDX
		conn.Execute(Sql)

		If Not FncIsBlank(THUMB_IMG) Then
			Sql = "	Update bt_menu_parent_file Set file_path='/thumbnail/', file_name='"& THUMB_IMG &"', file_ext='', mod_user_idx='"& SITE_ADM_CD &"', mod_date=GetDate(), mod_ip='"& REG_IP &"' Where menu_idx = " & MIDX & " And file_type = 'THUMB' "
		...
		If Not FncIsBlank(MAIN_IMG) Then
			Sql = "	Update bt_menu_parent_file Set file_path='/pc/', file_name='"& MAIN_IMG &"', file_ext='', mod_user_idx='"& SITE_ADM_CD &"', mod_date=GetDate(), mod_ip='"& REG_IP &"' Where menu_idx = " & MIDX & " And file_type = 'MAIN' "
			conn.Execute Sql, Uplow
			If Uplow > 0 Then 
			Else 
				Sql = "	Insert Into bt_menu_parent_file(menu_idx, file_type, file_name, file_ext, file_path, del_yn, reg_user_idx, reg_date, reg_ip) " & _
		...
		If Not FncIsBlank(MOBILE_IMG) Then
			Sql = "	Update bt_menu_parent_file Set file_path='/mobile/', file_name='"& MOBILE_IMG &"', file_ext='', mod_user_idx='"& SITE_ADM_CD &"', mod_date=GetDate(), mod_ip='"& REG_IP &"' Where menu_idx = " & MIDX & " And file_type = 'MOBILE' "
			conn.Execute Sql, Uplow
			If Uplow > 0 Then 
			Else 
				Sql = "	Insert Into bt_menu_parent_file(menu_idx, file_type, file_name, file_ext, file_path, del_yn, reg_user_idx, reg_date, reg_ip) " & _
