...
<%
	...
	Detail = "&CD="& CD & "&ORD="& ORD & "&MTP="& MTP & "&USE="& USE & "&LNUM="& LNUM & "&SM="& SM & "&SW="& Server.UrlEncode(SW)
...
function CopyMenu(){
	...
	document.location.href="menu_parent_form.asp?CD=<%=CD%>&SIDX="+SIDX;
}
function ChangeOrder(MENU_IDX, FVAL){
	$.ajax({
		async: false,
		type: "POST",
		url: "menu_parent_ord_proc.asp",
					...
					<tr>
						<th>
							<div class="board_select">
								...
								<%End If%>
									<select name="USE" id="USE" onChange="document.location.href='?CD=<%=CD%>&SM=<%=SM%>&SW=<%=Server.UrlEncode(SW)%>&ORD=<%=ORD%>&LNUM=<%=LNUM%>&MTP=<%=MTP%>&USE='+this.value">
...
%>
								<tr>
                                    ...
                                    <td><input type="button" value="수정" class="btn_white" onClick="document.location.href='menu_parent_form.asp?MIDX=<%=menu_idx%>&PAGE=<%=PAGE%><%=Detail%>'"></td>
                                </tr>
<%
				num = num - 1
				Rlist.MoveNext
...
<!-- #include virtual="/inc/paging.asp" -->
                        <div style="display:inline-block;float:right;">
                            ...
                            <button type="button" class="btn_white125" onClick="document.location.href='menu_parent_form.asp?CD=<%=CD%>'">등록</button>
                        </div>
                    </div>
                </div>
              </div>  
