...
'		If CD = "B" Then BCD = "BA"	'닭익는마을인 경우 '명가식' 을 기본값으로
		...
		exp5_yn		= "N"
	Else
		Sql = "Select * From bt_menu_parent Where menu_idx = " & MIDX
...
function InputCheck(){
	...
	$.ajax({
		async: true,
		type: "POST",
		url: "menu_parent_form_proc.asp",
		data: $("#inputfrm").serialize(),
		dataType: "text",
		success: function (data) {
			alert(data.split("^")[1]);
			if(data.split("^")[0] == 'Y'){
				document.location.href='menu_parent.asp?<%=Detail%>';
			}
		},
...
function DeleteID(MIDX){
	if(confirm('해당 메뉴를 삭제 하시겠습니까?')){
		$.ajax({
			async: false,
			type: "POST",
			url: "menu_parent_form_dproc.asp",
			data: {"CD":"<%=CD%>","MIDX":MIDX},
			dataType : "text",
			success: function(data) {
				alert(data.split("^")[1]);
				if (data.split("^")[0] == "Y") {
					document.location.href='menu_parent.asp?CD=<%=CD%>';
				}
			},
			error: function(data, status, err) {
				alert(err + '서버와의 통신이 실패했습니다.');
...
<script type="text/javascript">
	...
	function option_parent_add()
	{
		...
		html = '';
		html += '<table style="border-top:solid 1px #000;" id="menu_option_table_'+ option_parent_num +'">';
		html += '	<tr>';
		...
		html += '		<td>';
		html += '			<input type="hidden" name="menu_option_idx" id="menu_option_idx_'+ option_parent_num +'" >';
		html += '			<input type="text" name="menu_option_number" id="menu_option_number_'+ option_parent_num +'" size="50">';
		html += '			<a href="javascript: option_parent_del('+ option_parent_num +')" class="btn_white125" style="width:70px; text-align:center; color:#ff0000;">삭제</a>';
		html += '		</td>';
		html += '	</tr>';
		html += '</table>';
		...
		$('#menu_option_number_'+ option_parent_num).autocomplete({
			minLength: 0,
			source: projects,
			focus: function (event, ui) {
//				$('#menu_option_number_'+ option_parent_num).val(ui.item.wr_subject_str);
				return false;
			},
			select: function (event, ui) {
//				console.log($('#'+ event.target.id).parent())
				$('#'+ event.target.id).parent().find('[name="menu_option_idx"]').val(ui.item.menu_idx);
				$('#'+ event.target.id).val(ui.item.menu_name);

				$( 'html, body' ).animate( { scrollTop : 0 }, 0 ); // 맨위로 ㄱㄱ
						...
						<table>
							<tr>
								<th>
									묶음 옵션
									<br>
									<a href="javascript: option_parent_add();" class="btn_white125" style="width:70px; color:#0000ff;">추가</a>
...
<%
	...
	If Not FncIsBlank(MIDX) And FncIsBlank(SIDX) Then
		Sql = "Select file_type, file_name From bt_menu_parent_file where menu_idx='"& MIDX &"'"
...
%>
							<tr>
								...
								<td colspan="3">
									...
									<span>제품목록/장바구니/주문내역페이지 -이미지 사이즈(단위:픽셀)-가로 1920px, 세로600px, 확장자 jpg <%If Not FncIsBlank(THUMB_IMG) Then%><%=THUMB_IMG%><%End If%></span>
								</td>
							</tr>
							<tr>
								<th>상세보기상단이미지(PC)</th>
								<td colspan="3">
									...
									<span>상세보기/메인페이지 -이미지 사이즈(단위:픽셀)-가로 1920px, 세로600px, 확장자 jpg <%If Not FncIsBlank(MAIN_IMG) Then%><%=MAIN_IMG%><%End If%></span>
								</td>
							</tr>
							<tr>
								<th>상세보기상단이미지(Mob)</th>
								<td colspan="3">
									...
									<span>상세보기/메인페이지 -이미지 사이즈(단위:픽셀)-가로 1920px, 세로600px, 확장자 jpg <%If Not FncIsBlank(MOBILE_IMG) Then%><%=MOBILE_IMG%><%End If%></span>
								</td>
							</tr>
							<tr>
								<th>옵션</th>
