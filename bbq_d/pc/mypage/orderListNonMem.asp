...
<%
	confirm_mobile = GetReqStr("cmobile","")
	pageSize = GetReqNum("pageSize", 10)
	gotoPage = GetReqNum("gotoPage", 1)

	params = "cmobile=" & confirm_mobile
%>
</script>
</head>
	...
	function chk_sms() {
		...
		$.ajax({
			...
			success: function(res) {
				showAlertMsg({msg:res.message, ok: function(){
					if(res.ChkCode == "C") {
						location.href = "orderListNonMem.asp?cmobile="+mobile;
					}
				}});
			}
		});
	...
	<div class="container">
		...
		<article class="content">
			...
			<div class="nonMem-top">
				...
				<p>
					주문 취소는 해당 가맹점에서만 취소 가능합니다. <br/>전화 주문 내용은 확인이 불가능합니다.
				</p>
				<ul>
...
<%
	...
	With cmd
		...
		.Parameters.Append .CreateParameter("@mobile", adVarChar, adParamInput, 20, confirm_mobile)
...
%>
							<tr>
								<td>
									...
									<a href="./orderView.asp?oidx=<%=rs("order_idx")%>&cmobile=<%=confirm_mobile%>&gotoPage=<%=gotoPage%>" class="btn btn-sm btn-grayLine">주문상세보기</a>
								</td>
								<td class="img">
