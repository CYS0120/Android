...
<%
	...
	If order_type = "" Or (order_type = "D" And (addr_idx = "" Or branch_id = "")) Or (order_type = "P" And branch_id = "") Or cart_value = "" Then
%>
...
<%
	...
	reqMem.mMerchantCode = PAYCO_MEMBERSHIP_MERCHANTCODE
...
function chkUsePoint() {
	...
	if(u_point > a_point) {
		...
		if(getTotalAmount() - getOtherCardUsePoint("") - u_point < 0) {
			setMaxPoint();
			showAlertMsg({msg:"결제금액을 초과하여 사용할 수 없습니다."});
			return false;
		} else {
...
function chkCardPoint(cno) {
	...
	if(u_point > a_point) {
		...
		if(getTotalAmount() - getOtherCardUsePoint(cno) - getSaveUsePoint() - u_point < 0) {
			useMaxCardPoint(cno);
			showAlertMsg({msg:"결제금액을 초과하여 사용할 수 없습니다."});
			return false;
		} else {
...
function makeOrder() {
	if(getTotalAmount() - getOtherCardUsePoint("") - getSaveUsePoint() < 0) {
		showAlertMsg({msg:"결제금액이 잘못되었습니다."});
		return;
	}
<%If CheckLogin() Then%>
	if(chkUsePoint()) {
		$("#o_form input[name=save_point]").val(getSaveUsePoint());
	} else {
		return false;
	}
	if(chkCardPointAll()) {
		$("#o_form input[name=bbq_card]").val(JSON.stringify(getCardUseList()));
	} else {
		return false;
	}
<%Else%>
...
<%End If%>
	...
	if($("#o_form input[name=order_idx]").val() != "" && $("#o_form input[name=order_num]").val() != "") {
		if(getSaveUsePoint() > 0 && $("#chk_mem").val() != "Y") {
			useMembership();
		} else {
			goPay();
		}
	...
	<%If CheckLogin() Then%>
					if(getSaveUsePoint() > 0 || getCardUseList().length > 0) {
						useMembership();
					} else {
						$("#chk_mem").val("Y");
						goPay();
...
<%
	...
	For i = 0 To iLen - 1
		...
		pItem.mTargetUnitPrice = cJson.get(i).value.price
		pItem.mUnitPrice = cJson.get(i).value.price
		pItem.mProductCount = cJson.get(i).value.qty

		reqMem.addProductList(pItem)

		totalAmount = totalAmount + ( cJson.get(i).value.price * cJson.get(i).value.qty )
		s_amt = s_amt + ( cJson.get(i).value.price * cJson.get(i).value.qty )
%>
						<tr>
							<td class="img">
							...
							<td class="info ta-l">
								<div class="pdt-info div-table">
									<dl class="tr">
										...
										<dd class="td pm"><%=cJson.get(i).value.qty%>개</dd>
										<dd class="td sum"><%=FormatNumber(cJson.get(i).value.price,0)%>원</dd>
									</dl>
<%
...
<%
	...
	reqMem.mOrderAmount = totalAmount
				...
				<table class="tbl-write">
					...
					<tbody>
						...
						<tr>
							...
							<td><input type="text" name="delivery_message" placeholder="<%If order_type = "P" Then%>방문자명 및 <%End If%>요청사항을 남겨주세요." class="w-600"></td>
						</tr>
					</tbody>
				</table>
			</div>
				...
				<table class="tbl-write">
					...
					<tbody>
						<tr>
							<th scope="row">쿠폰</th>
							<td>
								<span class="display-ib w-300"><input type="text" id="coupon_amt" name="coupon_amt" value="0" numberOnly readonly class="w-260"> 원</span>
								<button type="button" class="btn btn-md3 btn-brown" onclick="javascript:lpOpen('.lp_paymentCoupon');">쿠폰적용</button>
								 &nbsp;&nbsp;
								사용가능 쿠폰 : <strong><%=pCoupon.mTotalCount%></strong>장
							</td>
						</tr>
						<tr>
							<th scope="row">포인트</th>
							<td>
								<span class="display-ib w-300"><input type="text" id="use_point" name="use_point" onblur="changeUsePoint();" numberOnly value="" class="w-260"><input type="hidden" id="ava_point" value="<%=pPoint.mTotalPoint%>"> P</span>
								<button type="button" onclick="setMaxPoint();" class="btn btn-md3 btn-brownLine">전액사용</button>
								 &nbsp;&nbsp;
								사용가능 포인트 <strong><%=FormatNumber(pPoint.mTotalPoint,0)%></strong>P
							</td>
						</tr>
<%
...
<!-- 										<div class="order_menu order_lpCoupon">
								...
								<div class="btn-wrap two-up mar-t20">
									<button type="button" class="btn btn-lg btn-red"><span>적용</span></button>
						...
						<div class="lp-container">
							...
							<div class="lp-content">
								...
								<div class="privacy">
										...
										<dl>
											...
											<dd>
												<ul>
													...
													<li><a href="#link10">10.기타</a></li>
													<li><a href="#link11">11.고지의 의무</a></li>
												</ul>
											</dd>
										</dl>
									</div>
									...
									<div class="area">
										ο 본 방침은 2013년 3월 1일부터 부터 시행됩니다.<br/>
										<br/>
										<strong><em id="link1"></em>1. 수집하는 개인정보 항목 및 수집방법</strong><br/>
										ο 회사는 회원가입, 상담, 서비스 제공 등을 위하여 아래와 같은 개인정보를 수집하고 있습니다.<br/>
										<br/>
										가. 수집항목 <br/>
										ο 이름, 이메일, 전화번호, 휴대폰 번호<br/>
										ο 서비스 이용과정이나 사업처리 과정에서 아래와 같은 정보들이 자동으로 생성되어, 수집 될 수 있습니다.<br/>
										...
										회사의 개인정보관련 처리 직원은 담당자에 한정시키고 있고 이를 위한 별도의 비밀번호를 부여하여 정기적으로 갱신하고 있으며, 담당자에 대한 수시 교육을 통하여 네이버 개인정보 처리방침의 준수를 항상 강조하고 있습니다. <br/>
										...
										<br/>
										ο공고일자 : 2013년 3월 1일 <br/>
										ο시행일자 : 2013년 3월 1일 <br/>
									</div>
									...
									<div class="area">
										 <strong>제1조 [목적]</strong> <br/>
											...
											* 협력사 회원은 관리자의 승낙 후에 회원의 자격을 가짐.<br/>
											...
											<br/>
											1. (시행일) 본 약관은 2013년 3월 1일부터 시행합니다. <br/>
											<br/>
