...
<script type="text/javascript">
	...
	function goOrder() {
		switch($("#order_type").val()) {
			...
			case "P":
			if($("#branch_id").val() == "" || $("#branch_data").val() == "") {
				showAlertMsg({msg:"픽업 가능한 매장이 선택되지 않았습니다."});
			}
			break;
		}
	...
	<div class="container">
		...
		<article class="content">
			...
			<div class="btn-wrap two-up inner mar-t60">
				<a href="/menu/menuList.asp" class="btn btn-lg btn-redLine"><span>다른메뉴 더보기</span></a>
				<button type="button" class="btn btn-lg btn-red" onclick="javascript:goOrder();"><span id="btn_order"><%If order_type = "D" Then%>배달<%ElseIf order_type = "P" Then%>픽업<%End If%>주문하기</span></button>
			</div>
