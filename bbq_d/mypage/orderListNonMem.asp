...
<script type="text/javascript">
	function req_sms() {
		var mobile = $.trim($("#nm_m1").val())+$.trim($("#nm_m2").val())+$.trim($("#nm_m3").val());
...
<form id="ckmobile" name="ckmobile" method="post">
	...
	<div class="container">
		...
		<article class="content">
			<h1 class="ta-l line2 mar-b40">비회원 주문내역</h1>

			<div class="nonMem-top">
				<div class="img"><img src="/images/mypage/ico_orderchk.gif" alt=""></div>
				<p>
					주문 취소는 콜센터(1588-9282)에서만 취소 가능합니다. <br/>전화 주문 내용은 확인이 불가능합니다.
				</p>
				<ul>
					<li>1. 주문번호를 클릭하시면 주문내용을 확인하실 수 있습니다.</li>
					<li>2. 주문내역은 최근 한달만 조회 가능합니다. </li>
				</ul>
			</div>
		

			<section class="section btline mar-t40">
				<table class="tbl-write bor-bnone bor-tnone">
						...
						<tr>
							...
							<td>
								<div class="ui-group-email w-350 display-ib mar-r10 va-m">
									<span><input type="text" onlynum id="nm_m1" maxlength="3"></span>
									<span class="dash w-20">-</span>
									<span><input type="text" onlynum id="nm_m2" maxlength="4"></span>
									<span class="dash w-20">-</span>
									<span class="pad-l0"><input type="text" onlynum id="nm_m3" maxlength="4"></span>
								</div>
								<button type="button" onclick="req_sms();" class="btn btn-md3 btn-red va-m ">인증번호 전송</button>
							</td>
							<td class="ta-r">
								<input type="text" id="nm_num" class="w-310 va-m mar-r10" placeholder="인증번호 입력">
								<input type="hidden" id="chk_mobile" value="">
								<button type="button" onclick="chk_sms();" class="btn btn-md3 btn-black va-m ">확인</button>
							</td>
						</tr>
					</tbody>
				</table>
...
%>
							<tr>
								<td>
									<%=rs("order_date")%><br/>
									<p class="orderNum"><%=rs("order_num")%></p>
									<a href="./orderView.asp?oidx=<%=rs("order_idx")%>&gotoPage=<%=gotoPage%>" class="btn btn-sm btn-grayLine">주문상세보기</a>
								</td>
								<td class="img">
									<a href="#" onclick="javascript:return false;"><img src="<%=SERVER_IMGPATH%><%=rs("thumb_file_path")&rs("thumb_file_name")%>" width="80px" height="80px" onerror="this.src='<%=placeholder_img("80", "80", "")%>';"></a>
								</td>
								<td class="info ta-l">
									<p class="icon">
										<span class="ico-branch red">비비큐 치킨</span>
									</p>
									<p class="menuName"><%=rs("menu_name")%><%If rs("menu_count") > 1 Then%> <span>외 <%=rs("menu_count")-1%>개</span><%End If%></p>
								</td>
								<td><%=FormatNumber(rs("pay_amt"),0)%>원</td>
								<td><%=rs("order_type_name")%></td>
								<td><%=rs("branch_name")%><br/><%=rs("branch_tel")%></td>
								...
								<%Else%>
									<%If rs("order_type") = "D" Then%>
										<td class="delivery_step<%=rs("order_step")%>"></td>
									<%ElseIf rs("order_type") = "P" Then%>
										<td class="takeOut_step<%=rs("order_step")%>"></td>
									<%End If%>
								<%End If%>
							</tr>
<%
