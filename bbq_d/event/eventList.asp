...
<%
		Do Until vRs.EOF
			If vRs("imgname") <> "N" Then
%>
				<div class="box">
					<div class="img"><a href="/event/eventView.asp?eidx=<%=vRs("BIDX")%>&event=<%=eventGbn%>&gotoPage=<%=gotoPage%>"><img src="<%=SERVER_IMGPATH%>/bbsimg/<%=vRs("imgname")%>" alt=""></a></div>
					<div class="info">
						<p class="subject"><a href="/event/eventView.asp?eidx=<%=vRs("BIDX")%>&event=<%=eventGbn%>&gotoPage=<%=gotoPage%>"><%=vRs("title")%></a></p>
...
%>
			...
			<div class="event-list">
				<div class="nomore">
					...
					<div class="txt">현재 진행중인 이벤트가 없습니다.</div>
				</div>
			</div>
			<!-- //이벤트 목록 없을 때 -->
<%
