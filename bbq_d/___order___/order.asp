...
<%
	...
	Dim reqMem : Set reqMem = New clsReqOrderGetListForOrder
	reqMem.mCompanyCode = PAYCO_MEMBERSHIP_COMPANYCODE
	reqMem.mMerchantCode = branch_id
	'reqMem.mMerchantCode = PAYCO_MERCHANTCODE
	reqMem.mMemberNo = Session("userIdNo")
	reqMem.mAccountTypeCode = "SAVE"
	reqMem.mMerchantCode = branch_id

	If instr(Request.ServerVariables("HTTP_USER_AGENT"), "bbqiOS") > 0 Or instr(Request.ServerVariables("HTTP_USER_AGENT"), "bbqAOS") > 0 Then
		reqMem.mOrderChannel = "APP"
	Else
		reqMem.mOrderChannel = "WEB"
	End If


	Dim pPrdList : pPrdList = Array()
	...
	If order_type = "D" Then
		...
		vAddress = vAddrName & " / " &vMobile & " / (" & vZipCode & ") " & vAddressMain & " " & vAddressDetail
	ElseIf order_type = "P" Then
		' 휴대전화번호 회원정보로 셋팅
		If CheckLogin() Then
			If Session("userPhone") <> "" And Len(Session("userPhone")) > 10 Then
	...
	'payco 간편결제 데이터 추가...
		'vPayco_Seller = aJson.payco_seller
		'vPayco_Cpid = aJson.payco_cpid
		'vPayco_Itemcd = aJson.payco_itemcd
...
<meta name="Keywords" content="주문/결제, BBQ치킨">
<meta name="Description" content="주문/결제 메인">
<title>주문/결제 | BBQ치킨</title>
<%
...
function setPayMethod(paymethod) {
	...
	$("#pay_"+paymethod.toLowerCase()).addClass("on");
...
function setCouponUse(obj) {
	...
	//alert(obj.value + " " + order_amt + " " + Number("11.5"));
	if(obj.value != ""){
...
function coupon_apply() {
	...
	if ($("#c_Id").val() != "") {
		...
		$("#coupon_amt").val(addCommas($("#discount_amount").val()));
	}else{
		$("#coupon_no").val('');
		$("#coupon_id").val('');
		$("#coupon_amt").val('');
...
function checkPointCoupon() {
	...
	if(getTotalAmount() - getUseEventPoint() - getOtherCardUsePoint("") - getCouponAmt() - u_point < 0) {
		setMaxPoint();
		return false;
	} else {
		return true;
...
<%End If%>
	...
	pay_amt = order_amt  + delivery - discount;
...
<%End If%>
	...
	$("#calc_pay_amt").text(addCommas(pay_amt));
	
	if(pay_amt == 0) {
		$("#pay_card").prop("disabled", true);
		$("#pay_phone").prop("disabled", true);
		$("#pay_payco").prop("disabled", true);
		$("#pay_paycoin").prop("disabled", true);
		$("#pay_later").prop("disabled", true);
		$("#pay_cash").prop("disabled", true);

		$("#pay_card").removeClass("on");
		$("#pay_phone").removeClass("on");
		$("#pay_payco").removeClass("on");
		$("#pay_paycoin").removeClass("on");
		$("#pay_later").removeClass("on");
		$("#pay_cash").removeClass("on");
		...
		$("#pay_card").prop("disabled", false);
		$("#pay_phone").prop("disabled", false);
		$("#pay_payco").prop("disabled", false);
		$("#pay_paycoin").prop("disabled", false);
...
function makeOrder() {
	...
	if (pay_method=='Point' || pay_method=='Later' || pay_method=='ECoupon' || pay_method=='Cash'){
		ClickCheck = 1;
	}

	if(getTotalAmount() - getOtherCardUsePoint("") - getSaveUsePoint() - getCouponAmt() - getUseEventPoint() < 0) {
...
<%End If%>
	...
	if(!$("#sAgree :checkbox").is(":checked")) {
		showAlertMsg({msg:"결제내역을 확인하세요."});
		// alert("결제내역을 확인하세요.");
		ClickCheck = 0;
		return false;
	}
...
function goPay() {
	...
	switch (pay_method) {
		...
		case "Cash":
			...
			$("#o_form").attr("target", "_self");
			$("#o_form").attr("action", "orderComplete.asp");
			$("#o_form").submit();
			break;
	}
}
		...
		<form id="pay_form" name="pay_form" method="post">
			...
			<section class="section section-orderStep">
				<ul>
					...
					<li class="step2 on"><span>02 주문/결제</span></li>
					<li class="step3"><span>03 주문완료</span></li>
				</ul>
			</section>
			<!-- //주문단계 -->

			<h1 class="line">주문결제</h1>
...
<%
	...
	For i = 0 To iLen - 1
		...
		If Temp_EVENT_POINT > 0 Then
			...
			pItem.mProductClassCd = cJson.get(i).value.type
			pItem.mProductClassNm = "메인"
			pItem.mProductCd = cJson.get(i).value.idx
			pItem.mProductNm = cJson.get(i).value.nm
			pItem.mTargetUnitPrice = ProdUnitPrice - Temp_EVENT_POINT
			pItem.mUnitPrice = ProdUnitPrice - Temp_EVENT_POINT
			pItem.mProductCount = 1
	'		pItem.mCouponPin = cJson.get(i).value.pin

			reqMem.addProductList(pItem)
		...
		If Temp_Order_Qty > 0 Then 
			...
			pItem.mProductCd = cJson.get(i).value.idx
			pItem.mProductNm = cJson.get(i).value.nm
			pItem.mTargetUnitPrice = ProdUnitPrice
			pItem.mUnitPrice = ProdUnitPrice
	...
	'		pItem.mCouponPin = cJson.get(i).value.pin
		...
		s_amt = s_amt + ( ProdUnitPrice * Order_qty )
%>
						<tr>
							<td class="img">
								<a href="javascript:;"><img src="<%=cJson.get(i).value.img%>" width="120px" height="120px" onerror="this.src='http://placehold.it/120x120';"/></a>
							</td>
							<td class="info ta-l">
								<div class="pdt-info div-table">
									<dl class="tr">
										<dt class="td"><%=cJson.get(i).value.nm%></dt>
										<dd class="td pm"><%=cJson.get(i).value.qty%>개
<%		If CouponPin <> "" Then %>
										<br><font color='red'>[E-쿠폰]</font>
<%		End If %>
										</dd>
										<dd class="td sum"><%=FormatNumber(ProdUnitPrice,0)%>원</dd>
									</dl>
<%
		If JSON.hasKey(cJson.get(i).value, "side") Then
			For Each sKey In cJson.get(i).value.side.enumerate()
				...
				Set pItem = New clsProductList
				pItem.mProductClassCd = cJson.get(i).value.side.get(skey).type
				pItem.mProductClassNm = "사이드"
				pItem.mProductCd = cJson.get(i).value.side.get(skey).idx
				pItem.mProductNm = cJson.get(i).value.side.get(skey).nm
				pItem.mTargetUnitPrice = cJson.get(i).value.side.get(skey).price
				pItem.mUnitPrice = cJson.get(i).value.side.get(skey).price
				pItem.mProductCount = cJson.get(i).value.side.get(skey).qty

				reqMem.addProductList(pItem)

				totalAmount = totalAmount + (cJson.get(i).value.side.get(sKey).price * cJson.get(i).value.side.get(sKey).qty)
				s_amt = s_amt + (cJson.get(i).value.side.get(sKey).price * cJson.get(i).value.side.get(sKey).qty)
%>
									<dl class="tr">
										<dt class="td"><%=cJson.get(i).value.side.get(sKey).nm%></dt>
										<dd class="td pm"><%=cJson.get(i).value.side.get(skey).qty%>개</dd>
										<dd class="td sum"><%=FormatNumber(cJson.get(i).value.side.get(skey).price,0)%>원</dd>
									</dl>
<%
			Next
		End If
%>
							...
							<td class="pay"><%=FormatNumber(s_amt,0)%>원</td>
<%
		If i = 0 Then
			If order_type = "D" Then
%>
							<td class="move" rowspan="<%=iLen%>">배달비<br/><%=FormatNumber(vDeliveryFee,0)%>원</td>
<%
			End If
%>
							<td class="pay" rowspan="<%=iLen%>"><%=FormatNumber(s_amt+vDeliveryFee,0)%>원</td>
<%
		End If
%>
						</tr>
<%
	...
	reqMem.mOrderAmount = OrderAmount

	Session("sess_avap") = 0	'주문시 포인트 초과여부를 체크하기 위해서 설정
	Set resMem = New clsResOrderGetListForOrder
	'Response.write JSON.stringify(reqMem.toJson())
	If CheckLogin() Then
		Set resMem = OrderGetListForOrder(reqMem.toJson())
		
		'Set pCoupon = resMem.mCouponList
		'Set pPoint = resMem.mPointList

		Session("sess_avap") = resMem.mTotalPoint
	End if
%>
...
%>
						<tr>
							...
							<td><input type="text" name="delivery_message" placeholder="<%If order_type = "P" Then%>방문자명 및 <%End If%>요청사항을 남겨주세요." class="w-600" onkeyup="chkWord(this, 200);"></td>
						</tr>
					</tbody>
				</table>
			</div>
...
%>
						<tr>
							<th scope="row">축하 포인트</th>
							<td>
							<ul>
							    <li><span class="display-ib w-300"><input type="text" id="event_point" name="event_point" readonly class="w-260" value="<%=FormatNumber(EVENT_POINT,0)%>"></span>
								&nbsp;&nbsp;<span class="min">*축하 포인트는 이벤트 기간내 매일 1회 사용 가능.</span></li>
							</ul>
...
<%			End If 
		End If %>
						<tr>
							<th scope="row">즉시 사용 쿠폰</th>
							<td>
								<span class="display-ib w-300">
								<input type="hidden" id="coupon_no" name="coupon_no">
								<input type="hidden" id="coupon_id" name="coupon_id">
								<input type="text" id="coupon_amt" name="coupon_amt" value="0" numberOnly readonly class="w-260">
								원</span>
								<button type="button" class="btn btn-md3 btn-brown" onclick="javascript:lpOpen('.lp_paymentCoupon');">즉시사용</button>
								 &nbsp;&nbsp;
								사용가능 쿠폰 : <strong><%=ubound(resMem.mCouponList)+1%></strong>장 <span class="min">*5% 적립포인트 즉시사용 쿠폰을 사용하시면 할인됩니다.</span>
							</td>
						</tr>
						<tr>
							<th scope="row">포인트</th>
							<td>
							<ul>
							    <li><span class="display-ib w-300"><input type="text" id="use_point" name="use_point" onblur="changeUsePoint();" numberOnly value="" class="w-260"><input type="hidden" id="ava_point" value="<%=resMem.mTotalPoint%>"> P</span>
								<button type="button" onclick="setMaxPoint();" class="btn btn-md3 btn-brownLine">전액사용</button>
								 &nbsp;&nbsp;
								사용가능 포인트 <strong><%=FormatNumber(resMem.mTotalPoint,0)%></strong>P &nbsp;<span class="min">*포인트 최소 사용 단위는 100P 단위 입니다.</span></li>
<!--							    <li class="min">"포인트 최소 단위는 100포인트 입니다."</li>-->
							</ul>
...
%>
						<!--tr>
							<th scope="row">비비큐카드</th>
							<td>
								<span class="display-ib w-300"><input type="text" id="use_card_<%=pCard.mCardDetail(i).mCardNo%>" name="use_card_<%=pCard.mCardDetail(i).mCardNo%>" onblur="changeUseCard('<%=pCard.mCardDetail(i).mCardNo%>');" numberOnly value="" class="w-260"> 원</span>
								<button type="button" onclick="useMaxCardPoint('<%=pCard.mCardDetail(i).mCardNo%>');" class="btn btn-md3 btn-brownLine">전액사용</button>
								&nbsp;&nbsp;
								남은금액 <strong><%=FormatNumber(pCard.mCardDetail(i).mRestPayPoint, 0)%>원</strong><input type="hidden" id="ava_card_<%=pCard.mCardDetail(i).mCardNo%>" value="<%=pCard.mCardDetail(i).mRestPayPoint%>" name="ava_card_<%=pCard.mCardDetail(i).mCardNo%>">
								 /  <%=pCard.mCardDetail(i).mCardNo%>
							</td>
						</tr-->
<%
				End If
	...
	function chkSmsNum() {
		...
		$.ajax({
			...
			success: function(res) {
				...
				// lpOpen("#lp_alert");
				// $("#lp_alert .lp-msg").text(res.message);
				// alert(res.message);
			}
		});
	}
			...
			<!-- 비회원 -->
			<div class="section-item bor-solid-b pad-b30">
				<h4>비회원 본인인증</h4>
				<table class="tbl-write bor-bnone">
					<caption>비회원 본인인증</caption>
					<tbody>
						<tr>
							<th scope="row">- 휴대폰 번호</th>
							<td>
								<div class="ui-group-email w-350 display-ib mar-r10 va-m">
									<span><input type="text" id="nm_m1" onlynum maxlength="3"></span>
									<span class="dash w-20">-</span>
									<span><input type="text" id="nm_m2" onlynum maxlength="4"></span>
									<span class="dash w-20">-</span>
									<span class="pad-l0"><input type="text" id="nm_m3" onlynum maxlength="4"></span>
								</div>
								<button type="button" onclick="reqSms();" class="btn btn-md3 btn-red va-m">인증번호 전송</button>
							</td>
						</tr>
						<tr>
							<th scope="row">- 인증 번호</th>
							<td>
								<input type="text" class="w-350 va-m mar-r10" id="nm_num"><input type="hidden" id="chkMobile" value="">
								
								<button type="button" onclick="chkSmsNum();" class="btn btn-md3 btn-black va-m">확인</button>
							</td>
						</tr>
					</tbody>
				</table>
				...
				<p class="mar-t10">
					<label class="ui-checkbox w-250">
						<input type="checkbox" id="nm_policy">
						<span></span> <span class="red">(필수)</span>  이용약관 동의
					</label>

					<button type="button" class="btn btn-sm btn-grayLine" onclick="javascript:lpOpen('.lp_policy');">내용보기</button>
				</p>

				<p class="mar-t20">
					<label class="ui-checkbox w-250">
						<input type="checkbox" id="nm_privacy">
						<span></span> <span class="red">(필수)</span>  개인정보 처리방침 동의
					</label>

					<button type="button" class="btn btn-sm btn-grayLine" onclick="javascript:lpOpen('.lp_privacy');">내용보기</button>
				</p>
...
%>
			...
			<div class="section-item">
				<h4>결제방법</h4>
				<div class="div-table payment_method">
					<div class="tr">
						<div class="td lef">
...
<%
		If vUseDANAL = "Y" Then
%>
											<li><button type="button" id="pay_card" class="payment_choiceSel" onclick="javascript:setPayMethod('Card');"><span>신용카드</span></button></li>
											<li><button type="button" id="pay_phone" class="payment_choiceSel" onclick="javascript:setPayMethod('Phone');"><span>휴대전화 결제</span></button></li>
<%
		End If
		If SAMSUNG_USEFG = "Y" Then 
		Else 
			If vUsePAYCO = "Y" Then
%>
											<li><button type="button" id="pay_payco" class="payment_choiceSel" onclick="javascript:setPayMethod('Payco');"><span>페이코</span></button></li>
<%
			End If
		End If 
%>
...
%>
											<li><button type="button" id="pay_paycoin" class="payment_choiceSel" onclick="javascript:setPayMethod('Paycoin');"><span>페이코인</span></button></li>
<%
		End If
%>
...
%>
								<dl class="spot">
									...
									<dd class="spot-list">
										<ul>
											<li><button type="button" id="pay_later" class="payment_choiceSel" onclick="javascript:setPayMethod('Later');"><span>신용카드</span></button></li>
											<li><button type="button" id="pay_cash" class="payment_choiceSel" onclick="javascript:setPayMethod('Cash');"><span>현금</span></button></li>
											<!-- <p class="cash_text">
												※현금결제시 100원미만 단위는 거스름이 없습니다<br>
												Ex)14,425원=14,500원결제
											</p> -->
										</ul>
									</dd>
								</dl>
						...
						<div class="td rig">
							<div class="order_calc">
								<div class="top div-table">
									<dl class="tr">
										<dt class="td">총 상품금액
										</dt><dd class="td" id="calc_tot_amt"><%=FormatNumber(totalAmount,0)%>원</dd>
									</dl>
									<%If order_type = "D" Then%>
									<dl class="tr">
										<dt class="td">배달비
										</dt><dd class="td" id="calc_deli_fee"><%=Formatnumber(vDeliveryFee,0)%>원</dd>
									</dl>
									<%End If%>
									<dl class="tr">
										<dt class="td">할인금액
										</dt><dd class="td" id="calc_dc_amt">0원</dd>
									</dl>
								</div>
								<div class="bot div-table">
									<dl class="tr">
										<dt class="td">최종 결제금액</dt>
										<dd class="td"><strong class="red" id="calc_pay_amt"><%=FormatNumber(totalAmount+vDeliveryFee,0)%></strong><span>원</span></dd>
									</dl>
								</div>
							</div>
							<div class="agree_ing mar-t20">
								<label class="ui-checkbox" id="sAgree">
									<input type="checkbox">
									<span></span> 상기 결제 내용을 진행하겠습니다.
								</label>
							</div>
							<div class="mar-t20">
								<button type="button" onclick="javascript:makeOrder();" class="btn btn-lg btn-red w-100p">결제</button>
							</div>
		...
		<form id="o_form" name="o_form" method="post">
			...
			<!-- //결제방법 -->
			<script type="text/javascript">$("#total_amt").val(<%=totalAmount%>);$("table.tbl-cart tbody tr:eq(0) td.pay:last").text(addCommas(<%=totalAmount+vDeliveryFee%>)+"원");</script>
							...
							<div class="lp-content">
								...
								<!-- <section class="section section_coupon">
									<form action="" class="form">
										<input type="text" placeholder="쿠폰 번호 입력" class="w-100p">
										<button type="submit" class="btn btn-md3 btn-black">쿠폰번호인증</button>
									</form>
									<p class="txt mar-t20">
									10-35자 일련번호 .  “ - ”  제외, 알파벳 ( I ) => 숫자 ( 1 ), 알파벳 ( O ) => 숫자 ( 0 ) 로정확히 확인 후 입력하시기 바랍니다
									</p>
								</section> -->
								<!-- //쿠폰인증번호 등록 -->
...
%>
										...
										<input type="hidden" name="discount_amount" id="discount_amount" value="0">
											<!--div class="box div-table">
												<div class="tr">
													<div class="td img"><img src="<%'=cJson.get(i).value.img%>" width="95px" height="82px" onerror="this.src='http://placehold.it/95x82?text=1';" alt=""></div>
													<div class="td info">
														<div class="coupon">
															<dl>
											...
											<div class="mar-t25">
												<select name="coupon_apply" id="coupon_select" class="w-100p" onChange="javascript:setCouponUse(this)">
											<%
												If UBound(resMem.mCouponList)+1 >= 1 Then
												
											%>
													<option value="">적용할 쿠폰을 선택해주세요.</option>
											<%
													For j = 0 To UBound(resMem.mCouponList)
											%>
													<option value="<%=resMem.mCouponList(j).mCouponNo&"||"&resMem.mCouponList(j).mCouponId&"||"&resMem.mCouponList(j).mConditionAmount&"||"&resMem.mCouponList(j).mConditionAmountYn&"||"&resMem.mCouponList(j).mRateTypeCode&"||"&resMem.mCouponList(j).mRateValue&"||"&resMem.mCouponList(j).mMaxRateValue&"||"&resMem.mCouponList(j).mEachApplyYn&"||"&resMem.mCouponList(j).mBenefitTypeCode%>"><%=resMem.mCouponList(j).mCouponName & " ~" & left(resMem.mCouponList(j).mValidEndYmdt,4)&"-"&mid(resMem.mCouponList(j).mValidEndYmdt,5,2)&"-"&mid(resMem.mCouponList(j).mValidEndYmdt,7,2) %></option>
											<%
													Next
												Else
											%>
											...
											<div class="sale div-table mar-t20">
												<dl class="tr">
													<dt class="td">할인금액</dt>
													<dd class="td"><span class="red" id="coupon_discount_amt">0</span>원</dd>
												</dl>
											</div>
										</div>
<%
...
<!-- 										<div class="order_menu order_lpCoupon">
											<div class="box div-table">
												<div class="tr">
													<div class="td img"><img src="http://placehold.it/95x82?text=1" alt=""></div>
													<div class="td info">
														<div class="coupon">
															<dl>
																<dt>황금 올리브 치킨</dt>
											...
											<div class="sale div-table mar-t20">
												<dl class="tr">
													<dt class="td">할인금액</dt>
													<dd class="td"><span class="red">0</span>원</dd>
												</dl>
											</div>
										</div>
								...
								<div class="btn-wrap two-up mar-t20">
									<button type="button" id="btn_coupon_apply" class="btn btn-lg btn-red" onclick="coupon_apply();"><span>적용</span></button>
									<button type="button" class="btn btn-lg btn-grayLine" onclick="javascript:lpClose(this);"><span>취소</span></button>
								</div>
							...
							<div class="lp-content">
								...
								<div class="privacy">
									<div class="top">
										<dl>
											<dt>
												...
												회사는 개인정보 처리방침을 통하여 고객님께서 제공하시는 개인정보가 어떠한 용도와 방식으로 이용되고 있으며, 개인정보보호를 위해 어떠한 조치가 취해지고 있는지 알려드립니다.<br />
												회사는 개인정보 처리방침을 개정 또는 변경하는 경우 웹사이트 공지사항(또는 개별공지)을 통하여 공지할 것입니다.<br />
											...
											<dd>
												<ul>
													...
													<li><a href="#link5">5.개인정보 제공 (목적 외 사용 및 제3자에 대한 제공 및 공유)</a></li>
													<li><a href="#link6">6.개인정보의 처리위탁</a></li>
									...
									<div class="area">
										...
										ο 서비스 이용과정 및 사업 처리과정에서 수집될 수 있는 개인정보의 범위 : 서비스 이용기록, 접속 로그, 쿠키, 접속IP정보, 결제기록, 이용정지 기록<br />
										<br />
										나. 개인정보 수집 방법 <br />
										...
										<br />
										<strong><em id="link5"></em>5. 개인정보 제공 (목적 외 사용 및 제3자에 대한 제공 및 공유)</strong><br />
										...
										가. 당사는 고객의 개인정보를 사전 동의 없이 제3자에게 제공하지 않습니다. 다만, 정보 주체의 동의 혹은 법규의 특별한 규정에 해당하는 경우에만 제3자에게 제공합니다.<br/>
										나. 당사는 보다 나은 서비스 제공, 고객 편의 등을 위하여 동의하시는 고객에 한해서 아래와 같이 개인정보를 제3자에게 제공하고 있습니다.<br/>
										ο 제공대상 : 비비큐 법인, 참숯바베큐 법인, 우쿠야 법인, 닭익는마을 법인, 올떡 법인, 와타미 법인, 창업센터<br />
										ο 제공하는 항목 : 이름, 생년월일, 로그인ID, 비밀번호, 휴대전화번호, 이메일주소, 접속 로그, 쿠키, 접속 IP 정보<br />
										ο 제공받는 자의 목적 : 서비스 제공에 관한 계약 이행 및 서비스 제공에 따른 요금 정산, 콘텐츠 제공, 물품배송 또는 청구지 등으로의 발송, 민원처리, 공지사항 전달, 신규 서비스(제품) 개발, 이벤트 및 광고성 정보 제공 및 참여기회 제공, 접속 빈도 파악, 회원의 서비스 이용에 대한 통계 등<br />
										ο 보유 및 이용기간 : 회사와 동일<br />
										<br />
										<strong><em id="link6"></em>6. 개인정보의 처리위탁</strong><br />
										...
										<!--1) 수탁업체 - 지투아이넷 주식회사<br />
										위탁업무 내용 - 웹 유지관리<br />
										개인정보의 보유 및 이용기간 - 회원탈퇴 시 혹은 위탁계약 종료 시까지<br />
										2) 수탁업체 - 푸드테크 주식회사<br />
										위탁업무 내용 - POS 시스템 유지관리<br />
										개인정보의 보유 및 이용기간 - 회원탈퇴 시 혹은 위탁계약 종료 시까지<br />
										3) 수탁업체 - 패밀리점 <br />
										위탁업무 내용 - 제품 배송 및 고객 이벤트 진행<br />
										개인정보의 보유 및 이용기간 - 회원탈퇴 시 혹은 위탁계약 종료 시까지<br />-->
										...
										회사는 이용자들의 개인정보를 처리함에 있어 개인정보가 분실, 도난, 누출, 변조 또는 훼손되지 않도록 안전성 확보를 위하여 다음과 같은 기술적/관리적 대책을 강구하고 있습니다.<br />
										<br />
										가. 비밀번호 암호화<br />
										...
										회사의 개인정보관련 처리 직원은 담당자에 한정시키고 있고 이를 위한 별도의 비밀번호를 부여하여 정기적으로 갱신하고 있으며, 담당자에 대한 수시 교육을 통하여 엔에이치엔페이코 개인정보 처리방침의 준수를 항상 강조하고 있습니다. <br />
										라. 개인정보관리책임자 지정 및 운영<br />
										그리고 사내 개인정보관리 책임자 지정 등을 통하여 '제너시스' 개인정보 처리방침의 이행사항 및 담당자의 준수여부를 확인하여 문제가 발견될 경우 즉시 수정하고 바로 잡을 수 있도록 노력하고 있습니다. 단, 이용자 본인의 부주의나 인터넷상의 문제로 ID, 비밀번호 등 개인정보가 유출되어 발생한 문제에 대해 회사는 일체의 책임을 지지 않습니다.<br />
										<br />
			...
			-->
										...
										"㈜제너시스"에 링크되어 있는 웹사이트들이 개인정보를 수집하는 행위에 대해서는 본 "㈜ 제너시스" 개인정보 처리방침이 적용되지 않음을 알려 드립니다.<br />
										<br />
										<strong><em id="link12"></em>12.고지의 의무</strong> <br />
										현 개인정보 처리방침 내용 추가, 삭제 및 수정이 있을 시에는 개정 최소 7일전부터 홈페이지의 '공지사항'을 통해 고지할 것입니다. 다만, 개인정보의 수집 및 활용, 제3자 제공 등과 같이 이용자 권리의 중요한 변경이 있을 경우에는 최소 30일 전에 고지합니다.<br />
										<br />
									...
									<div class="area">
									 ...
									 <!--	9. 행복한집밥 (http://www. )<br/>-->
										 ...
										 * 협력사 회원은 관리자의 승낙 후에 회원의 자격을 가짐.<br/>
										 ...
										 ① 회사는 정보통신망법 등 관계 법령에 정하는 바에 따라 회원의 개인정보를 보호하기 위해 노력합니다. 개인정보의 보호 및 사용에 대해서는 관련법 및 회사의 개인정보 처리방침이 적용 됩니다. 다만 회사의 공식 사이트 이외의 링크된 사이트에서는 회사의 개인정보 처리방침이 적용되지 않습니다. 또한 회사는 회원의 귀책사유로 인해 노출된 정보에 대해서 일체의 책임을 지지 않습니다. <br/>
										 <br/>
