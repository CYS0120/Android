...
<%
	ecoupon_yn = GetReqStr("couponType", "")
%>
<!doctype html>
<html lang="ko">
<head>
...
function setScreen() {
	...
	switch($("#order_type").val()) {
		case "D":
			$(".section-item.item01").show();
			$(".section-item.item02").hide();
			$(".orderType-radio2").removeClass("on");
		break;
		case "P":
			$(".section-item.item01").hide();
			$(".section-item.item02").show();
			$(".orderType-radio2").addClass("on");
		break;
	}
}
</script>
...
<script type="text/javascript">
	function goOrder() {
		...
		// $("#cart_form").submit();
		<% If ecoupon_yn = "Y" Then %>
			$("#cart_form").attr("action", "ecoupon.asp");
		<% Else %>
...
<script type="text/javascript">
	function chkBranchSelect() {
		...
		if($.trim($("#branch_id").val()) != "") {
			...
			bhtml += "<td><strong class=\"red\">포장 매장</strong></td>\n";
			...
			$("#btn_store_search").text("포장매장 재검색");
			$("#btn_pickup_order").show();
		} else {
			bhtml = "<td colspan=\"5\" class=\"noData\">포장 매장 찾기를 통해 포장가능한 매장을 선택해 주세요.</td>";

			$("#btn_store_search").text("포장 매장 찾기");
				...
				if(res.result == "0000") {
					if($("#branch_id").val() == branch_id) {
						$.ajax({
							...
							data: {"MENUIDX":$("#CART_IN_PRODIDX").val(),"BRANCH_ID":branch_id},
							dataType: "json",
							success: function(res) {
								if(res.result == "9999") {
									...
									$("#cart_form").attr("action", "cart.asp");
									$("#spent_time").val($(".orderType-radio2 input[name=after]:checked").val());
									$("#cart_form").submit();
								}
							},
							error: function(xhr) {
	...
	<div class="container">
		...
		<article class="content">
			...
			<section class="section">
				...
				<div class="orderType-radio orderType-radio2<%If order_type = "P" Then%> on<%End If%>">
					...
					<label class="ui-radio">
						<input type="radio" name="after" value="45" id="after45">
						<span></span> 45분 후
					</label>
					<label class="ui-radio">
						<input type="radio" name="after" value="60" id="after60">
						<span></span> 60분 후
					</label>
					<label class="ui-radio">
...
<%
	If branch_data = "" Then
%>									
									<td colspan="5" class="noData">포장 매장 찾기를 통해 포장가능한 매장을 선택해 주세요.</td>
<%
	Else
...
%>
						...
						<div class="btn-wrap two-up inner mar-t60">
							<button type="button" class="btn btn-lg btn-black" onclick="lpOpen('.lp_shopList');"><span id="btn_store_search">포장 매장 찾기</span></button>
							...
							<div class="lp-content">
								<section class="section">
									<form id="form_addr" name="form_addr" method="post" onsubmit="return false;">
											...
											<tbody>
												...
												<tr>
													...
													<td>
														...
														<div class="mar-t10">
															<input type="text" class="w-100p" name="address_detail" maxlength="30">
														</div>
													</td>
												</tr>
											</tbody>
										...
										<div class="btn-wrap two-up pad-t20 bg-white">
											<button type="button" onclick="javascript:<% If CheckLogin() Then %>validAddress()<% Else %>validAddressNoMember()<% End If %>;" class="btn btn-lg btn-black btn_confirm"><span>확인</span></button>
