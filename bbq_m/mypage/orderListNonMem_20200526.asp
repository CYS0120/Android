...
<div class="wrapper">
	...
	<form id="ckmobile" name="ckmobile" method="post">
		...
		<input type="t" name="cmobilecheck" id="cmobilecheck">
	</form>
	...
	<div class="container">
		...
		<article class="content">
			...
			<div class="page_title">
				<img src="/images/mypage/ico_orderchk.png" alt="">
				<span>비회원 주문조회 </span>
			</div>
			...
			<div class="section-wrap section-orderInfo">
				<div class="nonMem-top">
					<p>
						주문 취소는 콜센터(1588-9282)에서만 취소 가능합니다. <br/>전화 주문 내용은 확인이 불가능합니다.
					</p>
					<ul>
						<li>1. 주문번호를 클릭하시면 주문내용을 확인하실 수 있습니다.</li>
						<li>2. 주문내역은 최근 한달만 조회 가능합니다. <a href="javascript:sms_reset()">[다시조회하기]</a></li>
					</ul>
				</div>
			...
			<div class="section-wrap section-orderInfo mar-t30">
				<section class="section">
					...
					<ul class="orderList orderList_item" id="order_list">
						...
						<li class="item">
							<a href="./orderViewNonMem.asp?oidx=<%=rs("order_idx")%>" class="item-order">
								<div class="item-header">
									<span class="branch ico-branch red">비비큐치킨</span>
									<span class="orderDate"><strong><%=rs("branch_name")%></strong> <em>|</em> <strong><%=rs("order_date")%></strong></span>
								</div>
								<h4 class="prod"><%=rs("menu_name")%><%If rs("menu_count") > 1 Then%> <em>외 <%=rs("menu_count")-1%>개</em><%End If%></h4>
								<div class="price"><span><%=FormatNumber(rs("order_amt")+rs("delivery_fee"),0)%></span>원</div>
								<%If rs("order_step") = "" Then %>
								<div></div>
								<%ElseIf rs("order_step") = "05" Then %>
								<div><%=rs("order_status_name")%></div>
								<%ElseIf rs("order_step") = "01" Or rs("order_step") = "02" Then %>
								<div class="state del_step<%=rs("order_step")%>"><%=rs("order_status_name")%></div>
								<%Else%>
								<%If rs("order_type") = "D" Then%>
									<div class="state del_step<%=rs("order_step")%>"><%=rs("order_status_name")%></div>
								<%ElseIf rs("order_type") = "P" Then%>
									<div class="state take_step<%=rs("order_step")%>"><%=rs("order_status_name")%></div>
								<%End If%>
							<%End If%>
							</a>
							<div class="btn-wrap">
								<ul class="in-sec">
									<li class="btn_1"><a href="javascript:;"><img src="/images/mypage/ico_order_basic.png" alt=""> <span class="red"><%=rs("order_type_name")%></span></a></li>
									<li class="btn_2"><a href="javascript:;"><img src="/images/mypage/ico_order_ok.png" alt=""> <span><%=rs("order_status_name")%></span></a></li>
								</ul>
							</div>
						</li>
						<%
		...
		<div class="lp-container">
			...
			<div class="lp-content">
				...
				<div class="privacy">
					...
					<div class="area">
						ο 본 방침은 2019년 4월 1일부터 부터 시행됩니다.<br /><br />

						<strong><em id="link1"></em>1. 수집하는 개인정보 항목 및 수집방법</strong><br />
						ο 회사는 회원가입, 상담, 서비스 제공 등을 위하여 아래와 같은 개인정보를 수집하고 있습니다.<br /><br />
						...
						나. 개인정보 수집 방법 <br />
						ο 홈페이지, 서면방식, 팩스, 전화, 상담게시판, 이메일 이벤트 응모, 배송요청<br /><br />

						<strong><em id="link2"></em>2. 개인정보의 수집 및 이용목적</strong> <br />
						ο 회사는 수집한 개인정보를 다음의 목적을 위해 활용합니다.<br /><br />
						...
						다. 마케팅 및 광고에 활용 신규 서비스(제품) 개발, 통계학적 특성에 따른 서비스 제공 및 광고 게재, 서비스의 유효성 확인, 이벤트 및 광고성 정보 제공 및 참여기회 제공, 접속 빈도 파악, 회원의 서비스 이용에 대한 통계 등<br /><br />
						<strong><em id="link3"></em>3. 개인정보의 보유 및 이용기간 </strong><br />
						ο 회사는 회원가입일로부터 서비스를 제공하는 기간 동안에 한하여 이용자의 개인정보를 보유 및 이용하게 됩니다. 그러나 회원탈퇴 등 개인정보 수집 및 이용목적이 달성된 후에는 해당 정보를 지체 없이 파기합니다. 단, 다음의 정보에 대해서는 아래의 이유로 명시한 기간 동안 보존합니다.<br /><br />
						...
						ο 소비자의 불만 및 분쟁처리에 관한 기록 : 보존기간 3년<br /><br />
						...
						ο 이용자가 회원가입 등을 위해 입력한 정보는 목적이 달성된 후 별도의 DB로 옮겨져(종이의 경우 별도의 서류함) 내부 방침 및 기타 관련 법령에 의한 정보보호 사유에 따라(보유 및 이용기간 참조)일정 기간 저장된 후 파기됩니다. <br /><br />

						나. 파기방법<br />
						ο 종이에 출력된 개인정보는 분쇄기로 분쇄하거나 소각을 통하여 파기합니다. <br />
						ο 전자적 파일 형태로 저장된 개인정보는 기록을 재생할 수 없는 기술적 방법을 사용하여 삭제합니다.<br /><br />
						...
						나. 법령의 규정에 의거하거나, 수사 목적으로 법령에 정해진 절차와 방법에 따라 수사기관의 요구가 있는 경우<br /><br />
						...
						※ 일부 서비스는 외부 콘텐츠 제공사(CP)에서 결제 및 환불 등에 대한 고객상담을 할 수 있습니다.<br /><br />
						...
						다만, 그러한 경우 서비스의 일부 또는 전부 이용이 어려울 수 있습니다.<br /><br />

						이용자의 개인정보 조회나 수정을 위해서는 '개인정보변경'(또는 '회원정보수정' 등) 버튼을, 가입해지(동의철회)를 위해서는 '회원탈퇴' 버튼을 클릭하여 본인 확인 인증 절차를 거치신 후에 직접 정보의 열람, 정정 또는 회원탈퇴가 가능합니다. <br /><br />

						사정이 여의치 않을 경우에는 개인정보관리책임자에게 서면, 전화 또는 이메일로 연락해 주시면 지체 없이 조치하겠습니다. <br /><br />

						이용자가 개인정보의 오류에 대한 정정을 요청하신 경우에는 정정이 완료되기 전까지 당해 개인정보를 이용 또는 제3자(배송업체 등)에게 제공하지 않습니다. 또한 잘못된 개인정보를 제3자에게 이미 제공한 경우에는 정정 처리결과를 제3자에게 지체 없이 통지하여 정정이 이루어지도록 하겠습니다. <br /><br />

						회사는 이용자 혹은 이용자의 법정 대리인의 요청에 의해 해지 또는 삭제된 개인정보에 대하여 "회사가 수집하는 개인정보의 보유 및 이용기간"에 명시된 바에 따라 처리하고 그 외의 용도로 열람 또는 이용할 수 없도록 처리하고 있습니다. <br /><br />
						...
						가. 쿠키 등 사용 목적 <br />
						회원과 비회원의 접속 빈도나 방문 시간 등을 분석, 이용자의 취향과 관심분야를 파악 및 접속자취 추적, 각종 이벤트 참여 정도 및 방문 회수 파악 등을 통한 타겟 마케팅 및 개인 맞춤 서비스 제공 <br /><br />

						나. 쿠키의 설치/운영 및 거부<br />
						이용자는 쿠키 설치에 대한 선택권을 가지고 있습니다. 따라서, 이용자는 웹 브라우저에서 옵션을 설정함으로써 모든 쿠키를 허용하거나, 쿠키가 저장될 때마다 확인을 거치거나, 아니면 모든 쿠키의 저장을 거부할 수도 있습니다. 다만, 이용자가 쿠키 설정을 거부하였을 경우 서비스의 제공 및 이용에 어려움이 있을 수 있습니다. <br /><br />

						다. 쿠키 설치 허용 여부를 지정하는 방법(Internet Explorer의 경우)은 다음과 같습니다. <br />
						① [도구] 메뉴에서 [인터넷 옵션]을 선택합니다. <br />
						② [개인정보 탭]을 클릭합니다. <br />
						③ [개인정보취급 수준]을 설정하시면 됩니다. <br /><br />
						...
						그리고 사내 개인정보관리 책임자 지정 등을 통하여 '제너시스' 개인정보취급방침의 이행사항 및 담당자의 준수여부를 확인하여 문제가 발견될 경우 즉시 수정하고 바로 잡을 수 있도록 노력하고 있습니다. 단, 이용자 본인의 부주의나 인터넷상의 문제로 ID, 비밀번호 등 개인정보가 유출되어 발생한 문제에 대해 회사는 일체의 책임을 지지 않습니다.<br /><br />
						...
						직 위 : 전무<br />
						전 화 : 02-3403-9114<br />
						메 일 : 15154@bbq.co.kr <br /><br />

						이용자는 "㈜제너시스"의 서비스를 이용하며 발생하는 모든 개인정보보호 관련 민원을 개인정보관리책임자로 신고하실 수 있습니다. "(주)제너시스 그룹사 "는 이용자들의 신고사항에 대해 신속하게 충분한 답변을 드릴 것입니다.<br /><br />
						...
						수집된 정보는 "제너시스" 그룹 및 위탁관리 업체 이외 제 3자에게 제공되지 않습니다.<br /><br />

						<strong><em id="link11"></em>11.기타</strong><br />
						"㈜제너시스"에 링크되어 있는 웹사이트들이 개인정보를 수집하는 행위에 대해서는 본 "㈜ 제너시스" 개인정보취급방침이 적용되지 않음을 알려 드립니다.<br /><br />
