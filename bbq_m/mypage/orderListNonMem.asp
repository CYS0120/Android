...
<meta name="Keywords" content="비회원 주문조회, BBQ치킨">
<meta name="Description" content="비회원 주문조회">
<title>비회원 주문조회 | BBQ치킨</title>

</head>

<%
	cmobile = GetReqStr("cmobile","")
	pageSize = GetReqNum("pageSize", 20)
	gotoPage = GetReqNum("gotoPage",1)

	cmobilecheck = GetReqStr("cmobilecheck",1)

	If cmobilecheck = "Y" Then '다시 누르시에 세션 초기화
	cmobile = ""
	Session("ORDER_NOMEMBERHP") = ""
	End if

	
	If Not FncIsBlank(cmobile) Then 
		Session("ORDER_NOMEMBERHP") = cmobile
	ElseIf Not FncIsBlank(Session("ORDER_NOMEMBERHP")) Then 
		cmobile = Session("ORDER_NOMEMBERHP")
	End If
%>

<script type="text/javascript">
	function req_sms() {
		var mobile = $("#nm_m1").val()+$.trim($("#nm_m2").val())+$.trim($("#nm_m3").val());

		if(mobile == "" || !(mobile.length == 10 || mobile.length == 11)) {
			showAlertMsg({msg:"휴대폰 번호를 확인하세요."});
			return;
		}
	...
	function chk_sms() {
		...
		$.ajax({
			...
			data: {mobile: mobile, app_num: app_num},
			dataType: "json",
			success: function(res) {
				showAlertMsg({msg:res.message, ok: function(){
					if(res.ChkCode == "C") {
						var f = document.ckmobile;
...
<div class="wrapper">
	...
	<form id="ckmobile" name="ckmobile" method="post">
		<input type="hidden" name="cmobile">
		<input type="hidden" name="cmobilecheck" id="cmobilecheck">
	</form>
	...
	<div class="container">
		...
		<article class="content inbox1000">
			...
			<div class="section-wrap section-orderInfo">
				<div class="nonMem-top">
					<p>
						<strong class="point">주문 취소는</strong><br/> 콜센터<strong class="cllNum">(1588-9282)</strong>에서만 가능합니다. <br/>전화 주문 내용은 확인이 불가능합니다.
					</p>
					<div>
						<ul>
							<li>주문번호를 클릭하시면 주문내용을 확인하실 수 있습니다.</li>
							<li>주문내역은 최근 한달만 조회 가능합니다. <a href="javascript:sms_reset()">다시조회하기</a></li>
						</ul>
					</div>
				</div>
				<%If cmobile = "" Then '비회원 인증시 안보이게 처리 202003220 조과장%>
				<div class="section-header order_head">
					<h3>비회원 본인인증</h3>
				</div>
				<ul class="phone-cert" style="display: block;">
					<li>
						<span class="ui-group-tel">
							...
							<span class="dash">-</span>
							<span><input type="text" id="nm_m3" onlynum maxlength="4"></span>
						</span>
						<a href="javascript:req_sms();" class="btn btn_small3 btn-red">인증번호</a>
					</li>
					<li>
						<input type="text" id="nm_num" placeholder="인증번호 입력" class="w-100p">
						<input type="hidden" id="chk_mobile" value="">
						<button type="button" onclick="chk_sms();" class="btn btn_small3 btn-black">확인</button>
					</li>
				</ul>
...
<!-- 
 ...
 -->
				...
				<script type="text/javascript">
					var page = 1;
					var order_pageSize = 100;
					var NonMem = "Y";
					var NonMem_cmobile = "<%=cmobile%>";
			...
			<div class="lp-container">
				...
				<div class="lp-content">
					...
					<div class="privacy">
						<div class="top">
							<dl>
								<dt>
									...
									회사는 개인정보 처리방침을 개정 또는 변경하는 경우 웹사이트 공지사항(또는 개별공지)을 통하여 공지할 것입니다.<br /><br />
						...
						<div class="area">
							본 방침은 2019년 4월 1일부터 부터 시행됩니다.<br />
							<br />


							<strong><em id="link1"></em>1. 수집하는 개인정보 항목 및 수집방법</strong><br />
							회사는 회원가입, 상담, 서비스 제공 등을 위하여 아래와 같은 개인정보를 수집하고 있습니다.<br />
							<br />
							가. 수집항목 <br />
							<ul class="li_style1">
								<li>성명, 아이디, 비밀번호, 생년월일, 주소, 전화번호, E-Mail</li>
								<li>서비스 이용과정 및 사업 처리과정에서 수집될 수 있는 개인정보의 범위 : 서비스 이용기록, 접속 로그, 쿠키, 접속IP정보, 결제기록, 이용정지 기록</li>
							</ul>
							나. 개인정보 수집 방법 <br />
							<ul class="li_style1">
								<li>회사는 이용자가 개인정보보호정책과 이용약관의 각각의 내용에 대해 ‘동의’ 또는 ‘동의하지 않는다’버튼을 클릭할 수 있는 절차를 마련하여, ‘동의’버튼을 클릭한 경우 개인정보 수집에 대해 동의한 것으로 봅니다.</li>
								<li>홈페이지, 서면방식, 팩스, 전화, 상담게시판, 이메일 이벤트 응모, 배송요청</li>
							</ul>
							<br />
							...
							<ul class="li_style2">
								<li>가. 서비스 제공에 관한 계약 이행 및 서비스 제공에 따른 요금정산, 콘텐츠 제공, 물품배송 또는 청구지 등으로의 발송</li>
								<li>나. 회원 관리 회원제 서비스 이용에 따른 본인 확인(인증절차), 개인 식별, 불량회원의 서비스 부정 이용 방지와 비인가 사용의 방지, 만 14세 미만 아</li>
								<li>다. 마케팅 및 광고에 활용 신규 서비스(제품) 개발, 통계학적 특성에 따른 서비스 제공 및 광고 게재, 서비스의 유효성 확인, 이벤트 및 광고성 정보 제공 및 참여기회 제공, 접속 빈도 파악, 회원의 서비스 이용에 대한 통계 등</li>
							</ul>
							<br />


							<strong><em id="link3"></em>3. 개인정보의 보유 및 이용기간 </strong><br />
							회사는 회원가입일로부터 서비스를 제공하는 기간 동안에 한하여 이용자의 개인정보를 보유 및 이용하게 됩니다. 그러나 회원탈퇴 등 개인정보 수집 및 이용목적이 달성된 후에는 해당 정보를 지체 없이 파기합니다. 단, 다음의 정보에 대해서는 아래의 이유로 명시한 기간 동안 보존합니다.<br /><br />
							가. 회사 내부방침에 의한 정보보유 사유<br />
							<ul class="li_style1">
								<li>부정/불량이용기록 (부정/불량이용자의 개인정보 포함)</li>
								<li>보존이유 : 서비스의 부정 및 불량 이용 방지와 부정/불량이용자의 재가입 방지</li>
								<li>보존기간 : 1년</li>
							</ul>
							...
							<ul class="li_style1">
								...
								<li>소비자의 불만 및 분쟁처리에 관한 기록 : 보존기간 3년</li>
							</ul>
							<br />
							...
							가. 파기절차<br />
							회사는 원칙적으로 개인정부 수집 및 이용목적이 달성된 후에는 해당 정보를 지체없이 파기합니다, 파기절차 및 방법은 다음과 같습니다.
							<ul class="li_style1">
							...
							<ul class="li_style1">
								<li>제공대상 : 비비큐 법인, 참숯바베큐 법인, 우쿠야 법인, 닭익는마을 법인, 올떡 법인, 와타미 법인, 창업센터</li>
								<li>제공하는 항목 : 이름, 생년월일, 로그인ID, 비밀번호, 휴대전화번호, 이메일주소, 접속 로그, 쿠키, 접속 IP 정보</li>
								<li>제공받는 자의 목적 : 서비스 제공에 관한 계약 이행 및 서비스 제공에 따른 요금 정산, 콘텐츠 제공, 물품배송 또는 청구지 등으로의 발송, 민원처리, 공지사항 전달, 신규 서비스(제품) 개발, 이벤트 및 광고성 정보 제공 및 참여기회 제공, 접속 빈도 파악, 회원의 서비스 이용에 대한 통계 등</li>
								<li>보유 및 이용기간 : 회사와 동일</li>
							</ul>
							<br />
							...
							<table class="privacy_six_table mar-t10">
								<colgroup>
									<col width="25%">
									<col width="50%">
									<col width="25%">
								...
								<tr>
									...
									<td>
										<ul>
											<li>개인정보의 보관 및 유지관리</li>
											<li>(주)제너시스비비큐의 요청에 의한 공지사항/마케팅정보전송</li>
											<li>(Push / LMS / SMS / E-mail)</li>
											<li>시스템 장애 관련 대응 및 처리</li>
										</ul>
									</td>
							...
							<ul class="li_style2">
								...
								<li>이용자의 개인정보 조회나 수정을 위해서는 '개인정보변경'(또는 '회원정보수정' 등) 버튼을, 가입해지(동의철회)를 위해서는 '회원탈퇴' 버튼을 클릭하여 본인 확인 인증 절차를 거치신 후에 직접 정보의 열람, 정정 또는 회원탈퇴가 가능합니다. </li>
								<li>사정이 여의치 않을 경우에는 개인정보관리책임자에게 서면, 전화 또는 이메일로 연락해 주시면 지체 없이 조치하겠습니다. </li>
								<li>
									이용자가 개인정보의 오류에 대한 정정을 요청하신 경우에는 정정이 완료되기 전까지 당해 개인정보를 이용 또는 제3자(배송업체 등)에게 제공하지 않습니다. 또한 잘못된 개인정보를 제3자에게 이미 제공한 경우에는 정정 처리결과를 제3자에게 지체 없이 통지하여 정정이 이루어지도록 하겠습니다.
								</li>
								<li>회사는 이용자 혹은 이용자의 법정 대리인의 요청에 의해 해지 또는 삭제된 개인정보에 대하여 "회사가 수집하는 개인정보의 보유 및 이용기간"에 명시된 바에 따라 처리하고 그 외의 용도로 열람 또는 이용할 수 없도록 처리하고 있습니다.</li>
							</ul>
							<br />
							...
							<ul class="li_style2">
								...
								<li>
									<span>다. 쿠키 설치 허용 여부를 지정하는 방법(Internet Explorer의 경우)은 다음과 같습니다.</span>
							...
							<ul class="li_style2">
								...
								<li>
									<span>다. 처리 직원의 최소화 및 교육</span>
									회사의 개인정보관련 처리 직원은 담당자에 한정시키고 있고 이를 위한 별도의 비밀번호를 부여하여 정기적으로 갱신하고 있으며, 담당자에 대한 수시 교육을 통하여 엔에이치엔페이코 개인정보 처리방침의 준수를 항상 강조하고 있습니다.
								</li>
								<li>
									<span>라. 개인정보관리책임자 지정 및 운영</span>
									그리고 사내 개인정보관리 책임자 지정 등을 통하여 '제너시스' 개인정보 처리방침의 이행사항 및 담당자의 준수여부를 확인하여 문제가 발견될 경우 즉시 수정하고 바로 잡을 수 있도록 노력하고 있습니다. 단, 이용자 본인의 부주의나 인터넷상의 문제로 ID, 비밀번호 등 개인정보가 유출되어 발생한 문제에 대해 회사는 일체의 책임을 지지 않습니다.
								</li>
							</ul>
							<br />
							...
							<ul class="li_style1">
								<li>공고일자 : 2019년 4월 1일 </li>
								<li>시행일자 : 2019년 4월 1일 </li>
							</ul>
						</div>
