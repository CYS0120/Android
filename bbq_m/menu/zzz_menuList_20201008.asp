				 ...
				 <div class="menu_nav_wrap">
					<nav class="menu_nav2">
						<div class="swiper-wrapper">
							...
							<span class="<% if anc = "103" then %>		on <% end if %> swiper-slide2"><button type="button" id="" onclick="menu_go('103'); ">모든 비비큐치킨</button></span>
							<span class="<% if anc = "105" then %>		on <% end if %> swiper-slide2"><button type="button" id="" onclick="menu_go('105'); ">네고왕</button></span>
							<span class="<% if anc = "106" then %>		on <% end if %> swiper-slide2"><button type="button" id="" onclick="menu_go('106'); ">메이플 버터갈릭</button></span>
							...
							<span class="<% if anc = "104" then %>		on <% end if %> swiper-slide2"><button type="button" id="" onclick="menu_go('104'); ">세트메뉴</button></span>
					...
					$(window).load(function(){
						setTimeout(function(){ 
							swiper = $('.menu_nav2').swiper({
								...
								<% if anc = "105" then %>		initialSlide: '1', <% end if %>
								<% if anc = "106" then %>		initialSlide: '2', <% end if %>

								<% if anc = "5" then %>			initialSlide: '3', <% end if %>
								<% if anc = "102" then %>		initialSlide: '4', <% end if %>
								<% if anc = "8" then %>			initialSlide: '5', <% end if %>
								<% if anc = "7" then %>			initialSlide: '6', <% end if %>
								<% if anc = "6" then %>			initialSlide: '7', <% end if %>

								<% if anc = "104" then %>		initialSlide: '8', <% end if %>
								<% if anc = "99999" then %>	initialSlide: '9', <% end if %>
								//freeMode:true
							});	
						}, 300);
					})
