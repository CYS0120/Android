...
<%
	...
	paycoin_start_date = "2020-10-21" ' 2020-07-24
	paycoin_end_date = "2020-10-21" ' 2020-08-01
	...
	Dim pmobile : pmobile = GetReqStr("mobile","")
	Dim p_order_idx : p_order_idx = GetReqStr("order_idx","")
	Dim p_order_num : p_order_num = GetReqStr("order_num","")
	Dim p_amt : p_amt = GetReqStr("amt","")
	p_amt = 1000
...
<%End If%>
		...
		$("#o_form input[name=order_idx]").val("<%=p_order_idx%>");
		$("#o_form input[name=order_num]").val("<%=p_order_num%>");
	...
	function goPay() {
		...
		switch (pay_method) {
			...
			case "Card":
				...
				$("#o_form").attr("action","/pay/cancel_danal_card/Ready.asp");
				$("#o_form").submit();
			...
			case "Phone":
				...
				$("#o_form").attr("action","/pay/cancel_danal/Ready.asp");
				$("#o_form").submit();
				setTimeout("ClickCheck = 0", 1000);
				break;
			// Payco 간편결제
			...
			case "Paycoin":
				if (bbq_mobile_type == "mobile") {
					$("#o_form").attr("action", "/pay/paycoin_cancel/ready_pre.asp");
					$("#o_form").submit();
				} else {
					<% If instr(Request.ServerVariables("HTTP_USER_AGENT"), "bbqiOS") > 0 Or instr(Request.ServerVariables("HTTP_USER_AGENT"), "bbqAOS") > 0 Then %>
					<% else %>
...
<div class="wrapper">
	...
	<div class="container">
		...
		<article class="content inbox1000">
			...
			<input type="hidden" name="mobile" value="<%=pmobile%>">
			...
			<div class="section-wrap">
				<section class="section section_orderDetail" id="payment_list">
						...
						<ul class="cart_list" >
							...
							<li class="cart_info">
								<dl>
									...
									<dd class="cart_choice" style="display:none">&nbsp;<span>기본 : <span><%=FormatNumber(ProdUnitPrice,0)%></span>원</span><!-- <%=cJson.get(i).value.qty%> --></dd>
									...
									<dd class="cart_num" style="display:none">
					...
					<div class="order_calc" style="display:none">
			...
			<!-- 포장,배달정보 -->
			<div class="section-wrap section-orderInfo" style="display:none">
				<div class="section-header order_head">
			...
			<!-- 쿠폰 및 포인트 사용 -->
			<div class="section-wrap section-payment">
				<div class="section-header order_head">
					<h3>쿠폰 및 포인트 사용</h3>
				</div>
				<div class="area border">
					...
					<dl>
						...
						<dd>
							...
							<input type="text" id="coupon_amt" name="coupon_amt" value="0" numberOnly readonly style="width:80px; margin-right:5px"> <button type="button" onclick="javascript:lpOpen('.lp_paymentCoupon');" class="btn btn-sm btn-grayLine">쿠폰적용</button>
						</dd>
					</dl>
					<dl>
						<dt>
						...
						<dd>
							<input type="text" id="use_point" maxlength="10" numberOnly value="" onblur="changeUsePoint();" style="width:80px; margin-right:5px"><input type="hidden" id="ava_point" value="<%=resOGLFO.mTotalPoint%>">
			...
			<!-- 비회원 -->
			<div class="section-wrap section-nonMem" style="display:none">
				<div class="section-header order_head">
					<h3>비회원 본인인증</h3>
				</div>
...
%>
				...
				<% if cdate(date) >= cdate(paycoin_start_date) and cdate(date) <= cdate(paycoin_end_date) then %><p class="payco_txt">※ 페이코인 결제시 50% 할인 (최대 10,000원)</p><% end if %>
			</div>
			<!-- //결제방법 -->
			...
			<div class="section-wrap section-orderInfo">
				...
				<div class="order_calc border" style="display:none">
				...
				<div class="order_calc border">
					...
					<div class="bot div-table">
						<dl class="tr">
							<dt class="td">최종 결제금액</dt>
							<dd class="td red" id="calc_pay_amt11"><%=FormatNumber(p_amt,0)%><span>원</span></dd>
						</dl>
					</div>
				</div>
			...
			<form id="o_form" name="o_form" method="post" action="orderComplete.asp">
				...
				<input type="hidden" name="pmobile" value="<%=pmobile%>">
			</form>
