...
<script type="text/javascript">
	...
	function go_next_page_map_P()
	{
		...
		for (i=0; i<orderType_arr.length; i++)
		{
			if (orderType_arr[i].checked)
			{
				location.href='/order/out_search.asp?order_type=' + orderType_arr[i].value
			}
		}
	}
...
%>
								<div class="box">
									...
									<%End If%>
									<ul class="info">
										<li><input type="radio" name="adress" value="<%=aRs("addr_idx")%>"></li>
										<li>
											<!-- <%=aRs("mobile")%> -->
...
%>
							...
							<% if cnt > 0 then %>
								<div class="btn-wrap">
									<button type="button" id="addr_btn" class="btn btn_small btn-gray" onclick="go_next_page()">주소 변경</button>
								</div>
							<% else %> 
								<div class="btn-wrap">
							...
							<div class="btn-wrap  mar-t20 nono">
								<button type="button" class="btn btn_middle btn-red" onclick="go_next_page_map()">매장 찾기</button>
							</div>
						...
						<div class="pickup-wrap"<%If order_type = "P" Then%> style="display:none"<%End If%>>
							...
							<div class="btn-wrap inner mar-t40">
								<button type="button" class="btn btn_small btn-gray" onclick="go_next_page_map_P()">포장매장 찾기</button>
							</div>

							<div class="btn-wrap  mar-t20 nono">
								<button type="button" class="btn btn_middle btn-red" onclick="go_next_page_map_P()">매장 찾기</button>
							</div>
						</div>
						<!--// 포장매장 매장 -->
