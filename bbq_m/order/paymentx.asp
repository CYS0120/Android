...
<%
	...
	Dim vBranchName, vBranchTel, vDeliveryFee, vBran, vSubCPID
...
%>
					<div class="order_menu">
						<div class="box div-table">
							<div class="tr">
								<div class="td img"><img src="<%=cJson.get(i).value.img%>" width="160px" height="160px" onerror="this.src='http://placehold.it/160x160?text=1';" alt=""></div>
								<div class="td info">
									<div class="sum">
										<dl>
											<dt><%=cJson.get(i).value.nm%></dt>
											<dd><%=FormatNumber(ProdUnitPrice,0)%>원 <span>/ <%=cJson.get(i).value.qty%>개</span>
<%		If CouponPin <> "" Then %>
...
%>
										<dl>
											<dt>- <%=cJson.get(i).value.side.get(skey).nm%></dt>
											<dd><%=FormatNumber(cJson.get(i).value.side.get(skey).price, 0)%>원 <span>/ <%=cJson.get(i).value.side.get(skey).qty%>개</span></dd>
										</dl>
<%
			Next
		End If
...
%>
					...
					<div class="order_calc">
						<div class="bot div-table">
							<dl class="tr">
								...
								<dd class="td"><%=FormatNumber(totalAmount,0)%><span>원</span></dd>
							</dl>
						</div>
					</div>
				</section>
			...
			<!-- //장바구니 리스트 -->
			<script type="text/javascript">$("#total_amt").val(<%=totalAmount%>);</script>
<%
			...
			<div class="section-wrap section-orderInfo">
				...
				<div class="area">
					<dl>
						...
						<dd><strong class="red"><%=vBranchName%></strong>(<%=vBranchTel%>)</dd>
					</dl>
					<dl>
						<dt><%=address_title%></dt>
						<dd><%If order_type = "D" Then%>
							<%=vAddrName%>  /  <%=vMobile%><br/>
							(<%=vZipCode%>) <%End If%><%=vAddress%>
						</dd>
					</dl>
<%
...
%>
					<dl>
						<dt>기타요청사항</dt>
						<dd><input type="text" name="delivery_message" placeholder="<%If order_type = "P" Then%>방문자명 및 <%End If%>요청사항을 남겨주세요." class="w-100p"></dd>
					</dl>
				</div>
			</div>
			<!-- 배달정보 -->
					...
					<dl>
						...
						<dd>
							<ul class="ui-pointUse">
								<li class="lef"><input type="text" id="event_point" name="event_point" maxlength="10" numberOnly readonly value="<%=FormatNumber(EVENT_POINT,0)%>" class="w-100p"></li>
							</ul>
						</dd>
					</dl>
<%			End If 
		End If %>
					<dl>
						<dt class="point_use">
							<span>쿠폰</span>
							<em>사용가능 쿠폰 : <strong><%=ubound(resOGLFO.mCouponList)+1%></strong>장</em>
						</dt>
						<dd>
							<ul class="ui-pointUse">
								<input type="hidden" id="coupon_no" name="coupon_no">
								<input type="hidden" id="coupon_id" name="coupon_id">
								<li class="lef"><input type="text" id="coupon_amt" name="coupon_amt" value="0" numberOnly readonly class="w-100p"></li>
								<li class="rig">
									<button type="button" onclick="javascript:lpOpen('.lp_paymentCoupon');" class="btn btn-md btn-grayLine w-100p btn_lp_open">쿠폰적용</button>
								</li>
							</ul>
						</dd>
					</dl>
					<dl>
						...
						<li>
							<span>포인트</span>
							<em>사용가능 포인트 : <strong><%=FormatNumber(resOGLFO.mTotalPoint,0)%>P</strong></em>
							</li>
							<li class="min">
							    * 포인트 최소 사용 단위는 100P 단위 입니다.
							</li>
						</ul>
						</dt>
						...
						<dd>
							<ul class="ui-pointUse">
								<li class="lef"><input type="text" id="use_point" maxlength="10" numberOnly value="" onblur="changeUsePoint();" class="w-100p"><input type="hidden" id="ava_point" value="<%=resOGLFO.mTotalPoint%>"></li>
								<li class="rig">
									<button type="button" onclick="setMaxPoint();" class="btn btn-md w-100p btn-grayLine">전액 사용</button>
								</li>
							</ul>
						</dd>
					</dl>
...
%>
			...
			<div class="section-wrap section-orderInfo">
				...
				<ul class="phone-cert mar-t30" style="display: block;">
					<li>
						...
						<a href="javascript:reqSms();" class="btn btn-md btn-red">인증번호</a>
					</li>
					<li>
						<input type="text" placeholder="인증번호 입력" id="nm_num" class="w-100p"><input type="hidden" id="chkMobile" value="">
						<button type="button" onclick="chkSmsNum();" class="btn btn-md btn-black">확인</button>
					</li>
				</ul>
				...
				<p class="mar-t60">
					...
					<a href="javascript:void(0);" class="fr black fs30 mar-t5"  onclick="javascript:lpOpen('.lp_policy');"><u>내용보기</u></a>
				</p>
				<p class="mar-t30">
					<label class="ui-checkbox">
					...
					<a href="javascript:void(0);" class="fr black fs30 mar-t5"  onclick="javascript:lpOpen('.lp_privacy');"><u>내용보기</u></a>
				</p>
			</div>
			<!-- //비회원 -->
<%
...
'vUsePAYCO = "Y"
	...
	If vUseDANAL = "Y" Or vUsePAYCO = "Y" Or vUseSGPAY = "Y" Then
%>
					<dl class="online">
						<dt>일반결제</dt>
						<dd>
							<ul>
<%
		If vUseSGPAY = "Y" Then		'SGPAY 추가 / Sewoni31™ / 2019.12.09
%>
			...
			<div class="section-wrap section-orderInfo bor-none">
				...
				<div class="order_calc">
					...
					<div class="bot div-table">
						<dl class="tr">
							<dt class="td">최종 결제금액</dt>
							<dd class="td" id="calc_pay_amt"><%=FormatNumber(totalAmount + vDeliveryFee,0)%><span>원</span></dd>
						</dl>
					</div>
				</div>
				<div class="agree_ing mar-t70">
					<label class="ui-checkbox">
				...
				<div class="lp-container">
					...
					<div class="lp-content">
						<form action="">
							...
							<!-- 상품별 쿠폰 적용 -->
							<section class="section section_orderDetail pad-t20">
								<div class="section-header order_head mar-b0">
									<h3 class="fl">상품별 쿠폰 적용</h3>
									<div class="fr txt-basic black mar-t10">사용가능 쿠폰 : <strong><%=couponCount%></strong>장</div>
								</div>
<%
	'For i = 0 To iLen - 1
%>
...
%>
							...
							<div class="btn-wrap two-up inner">
								<button type="button" class="btn btn-lg btn-black" onclick="javascript:coupon_apply();"><span>확인</span></button>
								<button type="button" onclick="javascript:lpClose('.lp_paymentCoupon');" class="btn btn-lg btn-grayLine"><span>취소</span></button>
							</div>
