...
<!--#include virtual="/order/member_setting_update.asp"-->
<%
	'===== 삼성이벤트 때문에 생성
	If CheckLogin() Then
...
function setGiftcardUse(obj) {
	...
	if(obj.value != ""){
	    ...
	    var giftcardAmt = array_giftcard[2]; // 상품권 가격 giftcard_amt
		var discount_amount = "0";
				...
				<div class="area border">
					...
					<dl id="test_giftcard">
                        ...
                        <dd>
                            ...
                            <input type="text" id="giftcard_amt" name="giftcard_amt" value="0" numberOnly readonly style="width:80px; margin-right:5px">
                             ...
                             <button type="button" onclick="javascript:Giftcard_ListCount();" class="btn btn-sm btn-grayLine">상품권적용</button>
                        </dd>
                    </dl>
					<dl>
						<dt >
...
<!-- Layer Popup : 상품권적용 -->
					...
					<form action="">
						<!-- 상품권인증번호 등록 -->
						 <section class="section section_coupon">
							...
							<form action="" class="form">
								<ul class="area">
									<li><input type="text" autocomplete="off" id="giftPIN" name="giftPIN" placeholder="상품권 번호 입력 ('-' 포함)" class="w-100p"></li>
									<li class="mar-t15"><button type="button" onclick="javascript:Giftcard_Check();" class="btn btn-lg btn-black w-100p">상품권번호인증</button></li>
								</ul>
							</form>
							<p class="txt mar-t20">
							- 알파벳 ( I ) =&gt; 숫자 ( 1 ), 알파벳 ( O ) =&gt; 숫자 ( 0 ) 로 정확히 확인 후 입력하시기 바랍니다.
	...
	<script type="text/javascript">
                ...
                success: function(res) {
                    if (res.result == 0) {
                        for (var i = 0; i < res.Count.length; i++){
                           rdata = "<option value='"+ res.Count[i].giftcard_idx + "||"+ res.Count[i].giftcard_number + "||"+ res.Count[i].giftcard_amt + "'>"+"지류상품권"+ (i + 1) +"</option>";
        ...
        function Giftcard_scan(){
            // document.location.href='/barcode/barcode_scan.asp';
	...
	<script type="text/javascript">
	    //현금영수증 선택 영역 시작 , (test M_1156_0 황올한닭발튀김)
	        if(sessionStorage.getItem("M_1156_0")){
	            $('#test_giftcard').show()
	        }else{
	            $('#test_giftcard').hide()
	        }
