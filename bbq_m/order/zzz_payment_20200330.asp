		...
		$.ajax({
			...
			success: function(res) {
				if(res.result == "0000") {
					...
					showAlertMsg({msg:res.message+"  매장리스트로 이동합니다", ok: function(){
	...
	<div class="container">
		...
		<article class="content">
			...
			<div class="page_title">
				<img src="/images/order/icon_card.png">
				<span>결제하기</span>
			</div>
							...
							<li class="cart_info">
								<dl>
									...
									<dd class="cart_num">
										...
										<span><%=FormatNumber(row_tot_price,0)%></span>원
									</dd>
								</dl>
							</li>
						</ul>
			...
			<script type="text/javascript">
				...
				if (<%=totalAmount_parent%> < 15000) {
					showAlertMsg({msg:"최소결제금액은 15,000원 이상 주문하셔야 됩니다.", ok: function(){
						history.back();
					}});
				}
			</script>
			...
			<div class="section-wrap section-orderInfo mar-t30">
				...
				<div class="area border">
					...
					<dl>
						<dt>기타요청사항</dt>
						<dd><input type="text" name="delivery_message" placeholder="<%If order_type = "P" Then%>방문자명 및 <%End If%>요청사항을 남겨주세요." class="w-100p" onkeyup="chkWord(this, 200);"></dd>
					</dl>
				</div>
			</div>
			<!-- 포장,배달정보 -->
				...
				<div class="area">
					...
					<dl>
						...
						<ul>
							...
							<li class="min"> *축하 포인트는 이벤트 기간내 매일 1회 사용 가능</li>
						</ul>
						</dt>
					...
					<dl>
						...
						<dd>
							<div class="ui-pointUse">
								<input type="hidden" id="coupon_no" name="coupon_no">
								<input type="hidden" id="coupon_id" name="coupon_id">
								<input type="text" id="coupon_amt" name="coupon_amt" value="0" numberOnly readonly style="width:100px; margin-right:5px"> <button type="button" onclick="javascript:lpOpen('.lp_paymentCoupon');" class="btn btn-sm btn-grayLine">쿠폰적용</button>
							</div>
						</dd>
					</dl>
					<dl>
						<dt class="point_use">
							...
							<p>* 최소 사용 단위는 100P 단위 입니다.</p>
						</dt>
						<dd>
							<div class="ui-pointUse">
								<input type="text" id="use_point" maxlength="10" numberOnly value="" onblur="changeUsePoint();" style="width:100px; margin-right:5px"><input type="hidden" id="ava_point" value="<%=resOGLFO.mTotalPoint%>">
...
'vUsePAYCO = "Y"
	...
	'테스트 계정만 SGPAY 노출/비노출 되도록 임시 처리 / Sewoni31™ / 2020.01.21
	If SGPayOn <> True Then
		vUseSGPAY = "N"
	End If

	'If vUseDANAL = "Y" Or vUsePAYCO = "Y" Then
	If vUseDANAL = "Y" Or vUsePAYCO = "Y" Or vUseSGPAY = "Y" Then		'SGPAY 추가(사용 가맹점 여부에 따라 노출/비노출) / Sewoni31™ / 2019.12.09
%>
					<dl class="online">
						<dt>일반결제</dt>
						<dd>
							<ul>
<%
		If vUseSGPAY = "Y" Then		'SGPAY 추가(사용 가맹점 여부에 따라 노출/비노출) / Sewoni31™ / 2019.12.09
%>
...
%>
			...
			<div class="section-wrap section-orderInfo bor-none mar-t30">
				...
				<div class="mar-t20">
					<button type="button" onclick="javascript:makeOrder();" class="btn btn_middle btn-red">결제</button>
				</div>
				...
				<form action="">
					...
					<section class="section section_orderDetail pad-t20">
						...
						<div class="btn-wrap two-up inner mar-t20">
							<button type="button" class="btn btn_middle btn-black" onclick="javascript:coupon_apply();">확인</button>
