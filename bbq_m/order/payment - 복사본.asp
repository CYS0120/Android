...
//return;
	...
	var u_coupon = [];
	if ($("#c_Id").val() != "") {
		u_coupon.push({couponNo:$("#c_No").val(), couponId: $("#c_Id").val(), discountAmount: $("#discount_amount").val()});
	}
	$("#use_coupon").val(JSON.stringify(u_coupon));
...
function checkPointCoupon() {
	...
	if(getTotalAmount() - getOtherCardUsePoint("") - getCouponAmt() - u_point < 0) {
		setMaxPoint();
		return false;
	} else {
		return true;
...
<%End If%>
	...
	function makeOrder() {
		...
		if(getTotalAmount() - getOtherCardUsePoint("") - getSaveUsePoint() - getCouponAmt() < 0) {
		...
		if($("#chk_mem").val()=="Y")
		{
			$.ajax({
						...
						$.ajax({
							...
							success: function(data) {
								...
								if(res.result == 0) {
									...
									//alert(res.order_num);
				<%If CheckLogin() Then%>
									if(getSaveUsePoint() > 0 || getCardUseList().length > 0 || getCouponAmt() > 0) {
										useMembership();
									} else {
										$("#chk_mem").val("N");
										goPay();
				...
				<%End If%>
						...
						showAlertMsg({msg:"이전에 사용된 멤버십이 취소되지 않았습니다."});
						$("#chk_mem").val("N");
					}
				},
				error: function(xhr) {
...
<%
	...
	For i = 0 To iLen - 1
		totalAmount = totalAmount + ( cJson.get(i).value.price * cJson.get(i).value.qty )'cJson.get(i).value.price
		...
		ppItem.mTargetUnitPrice = cJson.get(i).value.price
		ppItem.mUnitPrice = cJson.get(i).value.price
		ppItem.mProductCount = cJson.get(i).value.qty
								...
								<div class="td info">
									<div class="sum">
										<dl>
											...
											<dd><%=FormatNumber(cJson.get(i).value.price,0)%>원 <span>/ <%=cJson.get(i).value.qty%>개</span></dd>
										</dl>
<%
...
<%
	reqOGLFO.mOrderAmount = totalAmount

	Dim resOGLFO
	If Session("userIdNo") <> "" Then
		Set resOGLFO = OrderGetListForOrder(reqOGLFO.toJson())
	'Else
	'	Set resOGLFO = New clsResOrderGetListForOrder
	End If
							...
							<div class="area">
								...
								<table class="privacy_six_table">
									...
									<tr>
										...
										<td rowspan="5">회원탈퇴 시 혹은 위탁계약 종료 시까지</td>
									</tr>
									<tr>
