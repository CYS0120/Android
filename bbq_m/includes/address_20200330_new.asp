<style type="text/css">
	span.order_type {font-size:0.785em; background:#ff0000; color:#fff; border-radius:10px; padding:1px 10px; vertical-align:t; margin:0 10px 0 5px; }
				...
				<% if Session("userIdNo") <> "" then %>
					...
					%>
									...
									if (branch_data != "" && typeof(branch_data) != "undefined" && branch_data != "" && branch_data != null) {
										...
										console.log(2);
										console.log('<%=aRs("addr_idx")%>');
										// 기본주소지로 매장 자동선택
										next_page_gubun = "D";
										selectShipAddress_new('<%=aRs("addr_idx")%>');
										console.log(3);
									}
					<%
									first_add_yn = "Y"
			...
			<span id="cart_form_copy_span"></span>
			...
			<script type="text/javascript">
				$( document ).ready(function() {
					if ($('#branch_id').length > 0) {
						...
						cart_form_copy_html = "";
						cart_form_copy_html += '<form id="cart_form_addr" name="cart_form_addr" method="post" action="payment.asp">';
						cart_form_copy_html += '	<input type="hidden" name="order_type" id="order_type" >';
						cart_form_copy_html += '	<input type="hidden" name="branch_id" id="branch_id" >';
						cart_form_copy_html += '	<input type="hidden" name="branch_data" id="branch_data" >';
						cart_form_copy_html += '	<input type="hidden" name="addr_idx" id="addr_idx" >';
						cart_form_copy_html += '	<input type="hidden" name="cart_value">';
						cart_form_copy_html += '	<input type="hidden" name="addr_data" id="addr_data" >';
						cart_form_copy_html += '	<input type="hidden" name="spent_time" id="spent_time">';
						cart_form_copy_html += '</form>';

						$('#cart_form_copy_span').html(cart_form_copy_html)
					}
				});
			</script>
