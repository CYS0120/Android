...
'    req.SetRequestHeader "SOAPAction", "http://tempuri.org/IFIND_STORE_v3/Search_Partner_Key"
    ...
    Else
        ...
        If Not node Is Nothing Then
			...
			result_msg_str = node(0).text
        End If

        Set node = Nothing
        Set node = searchResult.getElementsByTagName("a:STORE_AREA")
        If Not node Is Nothing Then
            If xmlResult <> "" Then xmlResult = xmlResult &","
		...
		If result_str <> "0000" Then 
'			xmlResult = "{""result"":"""& result_str &""",""message"":"""& result_msg_str &"""}"
			xmlResult = "{""result"":"""& result_str &""",""message"":""근처에 배달 가능한 매장이 존재하지 않습니다.""}"
		Else 
